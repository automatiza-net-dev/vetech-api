...
export default class AuthController {
  ...
  public async controllerLogin({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(LoginValidator);
  ...
  public async whoAmI({ auth, response, request }: HttpContextContract) {
    ...
    const { user, unit_id, system_id, ip } = this.sharedService.extractUser(auth);
    if (ip) {
    ...
    if (user.type === "controller") {
      if (!unit_id) {
        return response.ok(await this.authService.getControllerWithoutUnit(user, system_id));
      }

      return response.ok(await this.authService.getControllerWithUnit(user, unit_id, system_id));
    }
    ...
    const { unit, system, group } = await this.sharedService.getAuthContext(auth);
    ...
    return response.ok({
      ...
      cl: await this.authService.getUserACL(user, system_id, unit.id, group.status === "Consulta"),
      isThirdParty: false,
    });
  }
