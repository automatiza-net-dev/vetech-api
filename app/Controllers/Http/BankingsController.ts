...
export default class BankingsController {
  ...
  async updateBanking({ auth, params, request, response }: HttpContextContract) {
    const { unit_id, user } = this.sharedService.extractUser(auth);
    const payload = await request.validate(UpsertBankingValidator);

    const result = await this.service.updateBanking(unit_id, user, params.id, payload);

    return response.ok(result);
  }

  async deleteBanking({ auth, request, response }: HttpContextContract) {
    await this.service.deleteBanking(
      await this.sharedService.getAuthContext(auth),
      request.param("id"),
    );
