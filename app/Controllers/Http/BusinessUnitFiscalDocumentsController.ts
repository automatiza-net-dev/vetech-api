...
import ResendNfseValidator from "App/Validators/FiscalDocument/ResendNfseValidator";
...
export default class BusinessUnitFiscalDocumentsController {
  ...
  public async store({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CreateBusinessUnitFiscalDocumentValidator);
    const { unit_id } = this.sharedService.extractUser(auth);
  ...
  public async authorize({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(AuthorizeBusinessUnitFiscalDocumentValidator);
  ...
  public async authorizeNfse({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(AuthorizeBusinessUnitNfseFiscalDocumentValidator);
  ...
  public async resendNfse({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(ResendNfseValidator);

    const result = await this.service.resendNfse(
      await this.sharedService.getAuthContext(auth),
      payload,
    );
  ...
  public async forceUpdateNfse({ auth, params, response }: HttpContextContract) {
    const { unit_id } = this.sharedService.extractUser(auth);
  ...
  public async cancelNfe({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CancelBusinessUnitFiscalDocumentValidator);
  ...
  public async cancelNfse({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CancelBusinessUnitServiceFiscalDocumentValidator);
    const { unit_id, user } = this.sharedService.extractUser(auth);
  ...
  public async disable({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(DisableBusinessUnitFiscalDocumentValidator);
    const { unit_id, user } = this.sharedService.extractUser(auth);
  ...
  public async correct({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CorrectBusinessUnitFiscalDocumentValidator);
    const { unit_id, user } = this.sharedService.extractUser(auth);
