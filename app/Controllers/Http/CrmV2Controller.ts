...
export default class CrmV2Controller {
  ...
  public async searchSyncableOpportunities({ auth, request, response }: HttpContextContract) {
  ...
  public async transferOpportunity({ auth, request, response }: HttpContextContract) {
  ...
  public async createOpportunity({ auth, request, response }: HttpContextContract) {
  ...
  public async searchActivities({ auth, request, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);
  ...
  public async searchOpportunities({ auth, request, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);

    const result = await this.service.searchOpportunities(authCtx, request.qs());

    return response.ok(result);
  }

  public async searchKanbanOpportunities({ auth, request, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);

    const result = await this.service.searchKanbanOpportunities(authCtx, request.qs());
