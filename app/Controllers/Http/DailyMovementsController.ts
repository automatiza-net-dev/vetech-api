...
export default class DailyMovementsController {
  ...
  public async index({ auth, request, response }) {
    const qs = request.qs();
    const dailyMovements = await this.service.index(await this.sharedService.getAuthContext(auth), {
      from: qs.from,
      to: qs.to,
    });
  ...
  public async openDailyMovement({ auth, request, response }: HttpContextContract) {
  ...
  public async closeDailyMovement({ auth, request, response, params }: HttpContextContract) {
  ...
  public async reopenDailyMovement({ auth, response, params }: HttpContextContract) {
  ...
  public async checkDailyMovement({ auth, request, response, params }: HttpContextContract) {
