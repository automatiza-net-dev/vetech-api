...
export default class HospitalizationMedicalPrescriptionsController {
  ...
  public async schedulingIndex({ auth, request, response }: HttpContextContract) {
    const { unit_id } = this.sharedService.extractUser(auth);
    const qs = request.qs();
  ...
  public async store({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CreateHospitalizationMedicalPrescriptionValidator);
  ...
  public async update({ auth, params, request, response }: HttpContextContract) {
    const payload = await request.validate(UpdateHospitalizationMedicalPrescriptionValidator);
    const { key } = this.service.matchSchema(payload.type, payload.frequency);
    // @ts-expect-error its fine
    const payload2 = await request.validate(MedicalPrescriptionValidation[key]);
  ...
  public async interruptPrescription({ auth, params, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);
  ...
  public async excludePrescription({ auth, params, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);
  ...
  public async updateSchedule({ auth, params, request, response }: HttpContextContract) {
