...
export default class HospitalizationOccurrencesController {
  ...
  public async store({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CreateHospitalizationOccurrenceValidator);
  ...
  public async storeAttachment({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(CreateHospitalizationOccurrenceAttachmentValidator);
  ...
  public async update({ auth, params, request, response }: HttpContextContract) {
    const payload = await request.validate(UpdateHospitalizationOccurrenceValidator);
    const { unit_id } = this.sharedService.extractUser(auth);
