...
export default class HospitalizationsController {
  ...
  public async completedIndex({ auth, request, response }: HttpContextContract) {
    const { unit_id } = this.sharedService.extractUser(auth);
  ...
  public async getHospitalizationScheduling({ auth, params, response }: HttpContextContract) {
    const authCtx = await this.sharedService.getAuthContext(auth);

    const result = await this.service.getHospitalizationScheduling(authCtx, params.id);
