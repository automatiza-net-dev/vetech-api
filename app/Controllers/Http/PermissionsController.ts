...
export default class PermissionsController {
  ...
  public async show({ params, response, auth }: HttpContextContract) {
    const { id } = params;
    return response.ok(await this.service.show(await this.sharedService.getAuthContext(auth), id));
  }

  public async fetchMenu({ response, auth }: HttpContextContract) {
    return response.ok(await this.service.fetchMenu(await this.sharedService.getAuthContext(auth)));
  }
  ...
  public async fetchScreens({ request, response, auth }: HttpContextContract) {
    ...
    return response.ok(
      await this.service.fetchScreens(await this.sharedService.getAuthContext(auth), payload),
    );
  }
  ...
  public async destroy({ params, response, auth }: HttpContextContract) {
    const { id } = params;
    await this.service.delete(await this.sharedService.getAuthContext(auth), id);
    return response.noContent();
  }
}
