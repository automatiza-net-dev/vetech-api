...
export default class RolesController {
  ...
  public async index({ request, response, auth }: HttpContextContract) {
    ...
    if (!ctx.unit_id) {
      return response.ok(
        await this.roleService.index(ctx.system_id, null, ctx.user.type, request.qs()),
      );
    }

    return response.ok(
      await this.roleService.index(ctx.system_id, ctx.unit_id, ctx.user.type, request.qs()),
    );
  }

  public async controllerIndex({ request, response, auth }: HttpContextContract) {
    const qs = request.qs();
  ...
  public async show({ params, response, auth }: HttpContextContract) {
    response.ok(
      await this.roleService.show(await this.sharedService.getAuthContext(auth), params.id),
    );
  }

  public async permissionSchematics({ request, response, auth }: HttpContextContract) {
    if (!auth.user) {
  ...
  public async permissionMetadata({ params, request, response, auth }: HttpContextContract) {
    if (!auth.user) {
      return new response.unauthorized();
    }
    ...
    if (auth.user instanceof User) {
      return response.ok(
        await this.roleService.rolePermissionMetadata(auth.user.system_id, params.id, {
          type: auth.user.type,
          newItems: request.qs().newItems,
        }),
      );
  ...
  public async storeController({ request, response, auth }: HttpContextContract) {
  ...
  public async addPermissions({ request, response, auth }: HttpContextContract) {
  ...
  public async managePermissions({ request, response, auth }: HttpContextContract) {
    const payload = await request.validate(ManageRolePermissionValidator);
    await this.roleService.manageRolePermissions(auth.user?.system_id ?? -1, payload);
  ...
  public async update({ params, request, response, auth }: HttpContextContract) {
    ...
    const updatedRole = await this.roleService.update(auth?.user?.system_id ?? -1, id, payload);

    return response.created(updatedRole);
  }

  public async updateController({ params, request, response, auth }: HttpContextContract) {
    return this.sharedService.errorHoc(response, async () => {
      const { id } = params;
      const payload = await request.validate(UpdateRoleValidator);
  ...
  public async destroy({ params, response, auth }: HttpContextContract) {
    const { id } = params;
    await this.roleService.delete(await this.sharedService.getAuthContext(auth), id);

    return response.noContent();
  }

  public async destroyController({ params, response, auth }: HttpContextContract) {
    return this.sharedService.errorHoc(response, async () => {
      const { id } = params;
  ...
  public async searchInfo({ request, response, auth }: HttpContextContract) {
    ...
    return response.ok(
      await this.roleService.searchRolePermissions(auth.user.system_id ?? -1, "user", request.qs()),
    );
  }

  public async searchControllerInfo({ request, response, auth }: HttpContextContract) {
    const qs = request.qs();
  ...
  public async copyRole({ request, response, auth }: HttpContextContract) {
    return this.sharedService.errorHoc(response, async () => {
      ...
      const result = await this.roleService.copyRole(auth.user?.system_id ?? -1, payload);
