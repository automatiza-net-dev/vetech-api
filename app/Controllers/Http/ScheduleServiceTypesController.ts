...
export default class ScheduleServiceTypesController {
  ...
  public async index({ auth, request, response }: HttpContextContract) {
    const qs = request.qs();
    const data = await this.service.index(await this.sharedService.getAuthContext(auth), {
  ...
  public async index2({ auth, request, response }: HttpContextContract) {
    const qs = request.qs();
    const data = await this.service.index2(await this.sharedService.getAuthContext(auth), {
      description: qs.description,
      group: qs.group,
    });
  ...
  public async show({ auth, params, response }: HttpContextContract) {
    const data = await this.service.show(await this.sharedService.getAuthContext(auth), params.id);
  ...
  public async store({ auth, request, response }: HttpContextContract) {
    ...
    const data = await this.service.store(await this.sharedService.getAuthContext(auth), payload);
