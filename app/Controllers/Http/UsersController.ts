...
export default class UsersController {
  ...
  public async resendConfirmationToken({ params, response }: HttpContextContract) {
    const { email } = params;
  ...
  public async createConfirmationToken({ request, response }: HttpContextContract) {
  ...
  public async confirmConfirmationToken({ request, response }: HttpContextContract) {
  ...
  public async handleChangePasswordEmail({ auth, response }: HttpContextContract) {
    await this.service.sendChangePasswordEmail(await this.sharedService.getAuthContext(auth));

    return response.noContent();
  }

  public async handleChangePassword({ auth, request, response }: HttpContextContract) {
  ...
  public async createUserController({ auth, request, response }: HttpContextContract) {
    try {
      const payload = await request.validate(CreateUserControllerValidator);
      await this.service.createUserController(auth.user?.system_id ?? -1, payload);
          ...
          validationErrors: e.messages.errors.reduce(
            (prev, curr) => {
              ...
              prev[curr.field].errors.push(
                curr.message.replace("Campo", `Campo '${UsersController.intlMap[curr.field]}'`),
              );
  ...
  public async updateUserController({ auth, request, response }: HttpContextContract) {
    try {
      const payload = await request.validate(UpdateUserControllerValidator);
      await this.service.updateUserController(auth.user?.system_id ?? -1, payload);
  ...
  public async fetchUserControllers({ auth, response }: HttpContextContract) {
    const data = await this.service.fetchUserControllers(auth.user?.system_id ?? -1);

    return response.ok(data);
  }

  public async deleteUserController({ auth, request, response }: HttpContextContract) {
    try {
  ...
  public async disableUserControllerRole({ auth, request, response }: HttpContextContract) {
  ...
  public async uploadUserSignature({ auth, request, response }: HttpContextContract) {
    const payload = await request.validate(UploadSignatureValidator);
    await this.service.uploadSignature(await this.sharedService.getAuthContext(auth), payload);
