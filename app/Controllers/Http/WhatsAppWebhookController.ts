...
export default class WhatsAppWebhookController {
  ...
  public async receiveTintim({ request, response }: HttpContextContract) {
    ...
    const cfg = await this.whatsappService.findConfigForTintim(body?.account?.code ?? v4());
    ...
    try {
      ...
      if (error instanceof ValidationException && request.body().event_type === "message.create") {
        return response.status(200).json({
          success: true,
  ...
  public async searchMessages({ auth, request, response }: HttpContextContract) {
    return response.ok(
