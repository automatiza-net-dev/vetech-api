...
export default class AttendanceService {
  ...
  public async index(unitId: string, data: ISearch) {
    const qb = Attendance.query().where("business_unit_id", unitId).preload("scheduleService");
  ...
  public async open(authCtx: AuthContext, data: ICreateTreatment) {
    if (!data.scheduleId && !data.patientId) {
      throw new BadRequestException("É preciso informar agendamento ou paciente", 400, "E_ERR");
    }
    ...
    return await Database.transaction(async (trx) => {
      ...
      if (data.scheduleId) {
        ...
        if (!mainTutor) {
          throw new BadRequestException("Paciente sem tutor ativo", 400, "E_ERR");
        }
        ...
        const validPatientSchedule = await Schedule.query()
          ...
          .whereHas("serviceStatus", (query) => {
            query.whereIn("type", ["AC", "AN", "ATR", "ATEND", "CIR", "OBS", "INT", "REC"]);
          })
