...
export default class BankingService {
  ...
  async storeBanking(authCtx: AuthContext, data: IUpsertBankingData) {
    return Database.transaction(async (trx) => {
      ...
      const fromCheckingAccount = await CheckingAccount.findOrFail(data.fromCheckingAccountId, {
        client: trx,
      });

      const toCheckingAccount = await CheckingAccount.findOrFail(data.toCheckingAccountId, {
        client: trx,
      });
      ...
      const total =
        data.documentValue + (data.feeValue || 0) - (data.discountValue || 0) - discount;
      ...
      const finance = await Finance.create(
        {
          ...
          installment: typeof data.installment !== "undefined" ? data.installment : 1,
  ...
  async updateBanking(unitId: string, user: User, id: string, data: IUpsertBankingData) {
    const banking = await Banking.query().where("id", id).where("business_unit_id", unitId).first();
    if (!banking) {
    ...
    const dailyCashier = await DailyCashier.query()
      ...
      .first();
    const checkingAccount = await CheckingAccount.findOrFail(data.fromCheckingAccountId);

    const discount = data.documentValue * (paymentMethod.fee / 100);

    const total = data.documentValue + (data.feeValue || 0) - (data.discountValue || 0) - discount;
    ...
    banking
      .merge({
        ...
        status: BankingStatus.B,
        prevBalance,
        balance: data.type === BankingType.C ? prevBalance + total : prevBalance - total,
        paymentMethodDiscountPercentage: paymentMethod.fee,
        paymentMethodDiscountValue: discount,
      })
      .save();
  ...
  public async deleteBanking(authCtx: AuthContext, bankingID: string) {
    ...
    await Database.transaction(async (trx) => {
      ...
      const banking = await Banking.query().useTransaction(trx).where("id", bankingID).first();
      if (!banking) {
