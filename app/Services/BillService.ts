...
where deposit_id = ?
  and product_variation_id = ?`,
	...
	async createBillWithTrx(
		...
		const createdItems = await Promise.all(tasks);
		...
		if (data.billType === "T" && data.receiptType === "T") {
			...
			const receiptTaxRules = await receiptTaxRulesQb;

			const receiptUfIcms = await UfIcms.query()
				.whereIn(
					"origin_uf",
					taxRules.map((rule) => rule.toUf),
				)
			...
			await newReceipt.related("items").createMany(
				createdItems.map((item) => {
					const variation = productVariations.find(
						(variation) => variation.id === item.product_variation_id,
					) as ProductVariation;

					const rule = receiptTaxRules.find(
						(rule) =>
					...
					const ufIcmsRule = receiptUfIcms.find(
						(ufIcms) =>
