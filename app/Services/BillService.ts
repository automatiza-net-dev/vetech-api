...
import ClientUsedCredit from "App/Models/ClientUsedCredit";
      ...
      const tasks = data.items
        ? data.items.map(async (elem) => {
            ...
            if (elem.billItemId) {
              ...
              // Usar createBillItemWithTrx para calcular impostos automaticamente
              await this.createBillItemWithTrx(trx, authCtx, {
                billId: bill.id,
        ...
        if (
          ...
          if (["51"].includes(cst)) {
            taxData = Object.assign(taxData, {
              ...
              icmsDeferredValue: icmsBase * ((billItem.taxRule?.icmsPercDiferimento ?? 0) / 100),
              icmsValue:
...
where deposit_id = ?
  ...
  async createBillPaymentForChunks(authCtx: AuthContext, data: ICreateBillPaymentData) {
    return Database.transaction(async (trx) => {
      ...
      const creditToUse = Decimal.min(creditoDisponivel, data.installmentsValue, totalToPay);
      ...
      const existingPayments = await BillPayment.query().whereIn(
        "bill_id",
        bills.map((b) => b.id),
      );

      const maxBlockPerBill = existingPayments.reduce(
        (acc, curr) => {
          if (!acc[curr.bill_id]) {
            acc[curr.bill_id] = 0;
          }

          acc[curr.bill_id] = Math.max(acc[curr.bill_id], curr.block);
      ...
      if (clientCredit && creditToUse.gt(0)) {
        ...
        await clientCredit
          ?.merge({
            usedValue: clientCredit.usedValue.plus(creditToUse),
          })
          .useTransaction(trx)
          .save();
        ...
        await ClientUsedCredit.create(
          {
            ...
            client_credit_id: clientCredit.id,
      ...
      await Promise.all(
        bills.map(async (bill) => {
          ...
          if (!data.creditOverflow) {
            ...
            if (isLastBill) {
              const diferenca = new Decimal(valorDescontarVendas).minus(totalDistribuido);
  ...
  async deleteClientPayment(authCtx: AuthContext, id: string) {
    await Database.transaction(async (trx) => {
      ...
      // Return credits used by this payment to the original ClientCredit
      if (payment.clientUsedCredits && payment.clientUsedCredits.length > 0) {
        for (const usedCredit of payment.clientUsedCredits) {
          // Find the original ClientCredit (the one that provided the credit)
          const originalCredit = await ClientCredit.query()
            .useTransaction(trx)
            .where("id", usedCredit.client_credit_id)
            .first();

          if (originalCredit) {
            // Return the used value to the original credit
            await originalCredit
              .merge({
                usedValue: originalCredit.usedValue.minus(usedCredit.usedValue),
              })
              .useTransaction(trx)
              .save();
          }

          // Mark the used credit as returned (soft delete)
          await ClientUsedCredit.query().useTransaction(trx).where("id", usedCredit.id).update({
            returned: true,
            deleted_at: DateTime.now(),
            reversed_by_user_id: authCtx.user.id,
          });
        }
      }

      if (payment.clientCredits.some((cc) => cc.usedValue.gt(new Decimal(0)))) {
      ...
      if (payment.paymentMethod.type === PaymentMethodType.CC) {
        for (const cc of payment.clientCredits) {
          if (deletedValue.eq(new Decimal(0))) {
            break;
          }

          if (cc.usedValue.eq(new Decimal(0))) {
            continue;
          }

          // Calcula quanto pode ser devolvido deste crédito
          // Sem deixar usedValue chegar a 0 (mantém pelo menos 0.01)
          const minRemaining = new Decimal(0.01);
          const maxReturn = cc.usedValue.minus(minRemaining);

          if (maxReturn.lte(0)) {
            continue;
          }

          const returnAmount = Decimal.min(deletedValue, maxReturn);

          await cc
            .merge({
              usedValue: cc.usedValue.minus(returnAmount),
            })
            .useTransaction(trx)
            .save();

          deletedValue = deletedValue.minus(returnAmount);
        }
      } else {
        if (payment.billPayments.some((bp) => bp?.finance?.status === FinanceStatus.B)) {
          throw new BadRequestException(
            "Registro financeiro marcado como baixado, não é possível excluir",
            400,
            "E_ERR",
          );
        }
...
where deposit_id = ?
    ...
    await Database.transaction(async (trx) => {
      ...
      await item.bill
        .merge({
          ...
          totalValue: new Decimal(totalProductValue).plus(totalServiceValue),
          icmsBase: validItems.reduce((acc, item) => acc + item.icmsBase, 0),
          icmsValue: validItems.reduce((acc, item) => acc + item.icmsValue, 0),
          icmsStBase: validItems
            .filter((i) => typeof i.icmsStValue === "number" && !Number.isNaN(i.icmsStValue))
  ...
  async recalculateItemsTaxes(authCtx: AuthContext, id: string) {
    await Database.transaction(async (trx) => {
      ...
      const ufIcms = await UfIcms.query()
        .useTransaction(trx)
        .whereIn("origin_uf", ufList)
        .whereIn("destination_uf", ufList)
        .first();

      const promises = bill.items.map(async (billItem) => {
        const totalValue = billItem.quantity
          .times(billItem.unitaryValue)
          .minus(billItem.discountValue ?? 0)
          .toNumber();

        const icmsBase =
          totalValue * ((100 - (billItem.taxRule?.icmsPercRedBaseCalculo ?? 0)) / 100);
        const icmsStBase_1 = this.isValidNumber(billItem.taxRule?.ivaIcmsSt)
          ? icmsBase + (icmsBase * (billItem.taxRule?.ivaIcmsSt ?? 0)) / 100
          : 0;
        const icmsStPercentageRedBase = this.isValidNumber(billItem.taxRule?.ivaIcmsSt)
          ? (billItem.taxRule?.icmsPercRedBaseCalculo ?? 0)
          : undefined;
  ...
  async createBillItemWithTrx(
    ...
    // Adicionar campos de partilha e diferimento que existem no createBillWithTrx
    toCreate = Object.assign(toCreate, {
      icmsDeferredValue: 0,
      icmsPartitionValue: 0,
      icmsPartitionOriginUfPercentage: rule?.icmsPerc,
  ...
  and product_variation_id = ?`,
	...
	async createBillPaymentForChunks(
		...
		return Database.transaction(async (trx) => {
			...
			const valorDescontarVendas = new Decimal(data.installmentsValue).minus(
				originalValue,
			);
			...
			await Promise.all(
				bills.map(async (bill) => {
					...
					if (!data.creditOverflow) {
						...
						if (isLastBill) {
							const diferenca = new Decimal(valorDescontarVendas).minus(
								totalDistribuido,
							);
					...
					Array.from({ length: installmentFee.installment }, (_, v) => {
						...
						return {
							...
							expirationDate: paymentMethod
								...
								: DateTime.now(),
							originalValue: new Decimal(installmentValue),
							value: installmentValue.minus(
								installmentValue.times(feeCtx).div(100),
							),
							totalValue: installmentValue.minus(
								installmentValue.times(feeCtx).div(100),
							),
							feeDiscountValue: installmentValue
								.minus(installmentValue.times(feeCtx).div(100))
	...
	async aggregateClientPayments(_authCtx: AuthContext, tutorID: string) {
		const rows: { total_value: string }[] = await Database.from(
			"client_credits",
		)
			.select(Database.raw("sum(original_value - used_value) as total_value"))
			.where("client_id", tutorID)
	...
	async deleteClientPayment(authCtx: AuthContext, id: string) {
		await Database.transaction(async (trx) => {
			const payment = await ClientPayment.query()
				...
				.preload("client")
				.preload("billPayments", (q) => q.preload("bill").preload("finance"))
				.preload("clientCredits", (q) =>
					q.whereRaw("(used_value < original_value) and reversed is false"),
				)
				.first();
			...
			const deletedValuePerBill = payment.billPayments.reduce(
				(acc, curr) => {
					...
					acc[curr.bill_id] = acc[curr.bill_id].plus(
						new Decimal(curr.totalValue),
					);
					return acc;
				},
			...
			for (const [billId, value] of Object.entries(deletedValuePerBill)) {
				const rowBill = await Bill.query()
					.useTransaction(trx)
					.where("id", billId)
					.firstOrFail();
			...
			if (payment.paymentMethod.type === PaymentMethodType.CC) {
				await payment.clientCredits
					.at(0)
					?.merge({
						usedValue: payment.clientCredits
							.at(0)
							?.usedValue.minus(deletedValue),
					})
					.useTransaction(trx)
					.save();
			} else {
				if (
					payment.billPayments.some(
						(bp) => bp?.finance?.status === FinanceStatus.B,
					)
				) {
					throw new BadRequestException(
						"Registro financeiro marcado como baixado, não é possível excluir",
