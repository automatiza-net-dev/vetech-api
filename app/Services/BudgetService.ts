...
export default class BudgetService {
  ...
  public async budgetsFromAttendance(authCtx: AuthContext, attendanceId: string) {
    return Budget.query()
  ...
  public async listOpenNegotiations(authCtx: AuthContext, patientId: string) {
    ...
    const statuses: { id: string; status: string }[] = await Database.from("budgets")
      .select(
        Database.raw(
          `id,
       case
    ...
    const departmentItemRows: {
      ...
      .select(
        Database.raw(
          "budget_items.id, budget_items.budget_id, d.id as department_id, d.description department_description, di.description department_item_description, di.id  as department_item_id, bid.observations",
        ),
      )
      .joinRaw(
      ...
      .whereIn(
        ...
        attendances.flatMap((a) => a.budgets.map((b) => b.id)),
      );
      ...
      attendances.map(async (elem) => {
        ...
        Object.assign(jsonObj, {
          budgets: elem.budgets.map((b) => ({
            ...
            items: b.items.map((row) => {
              ...
              return {
                ...jsonItem,
                departmentItems: departmentItemRows.filter(
                  (row) => row.budget_id === b.id && row.id,
                ),
              };
            }),
          })),
        ...
        Object.assign(jsonObj, {
          bills: bills.map((b) => ({
            ...
            cancelUser: this.sharedService.captureGroup(b.cancelUser, (sel) => ({
              id: sel.id,
              name: sel.name,
            })),
            finishCancelUser: this.sharedService.captureGroup(b.finishCancelUser, (sel) => ({
              id: sel.id,
              name: sel.name,
            })),
            ...
            departmentItems: billDepartmentItemRows.filter((ro) => ro.bill_id === b.id),
            payments: b.payments.map((p) => ({
              ...p.toJSON(),
              expiration_date: p.expirationDate ? p.expirationDate.toFormat("yyyy-MM-dd") : null,
            })),
          })),
        });
        ...
        if (elem.budgets.some((b) => b.status === BudgetStatus.C)) {
          const treatments = await Treatment.query()
            .where("economic_group_id", authCtx.group.id)
            .where("business_unit_id", authCtx.unit.id)
            .whereIn("bill_id", elem.budgets.map((b) => b.bill_id).filter(Boolean))
            .preload("items", (query) => {
              // query.whereNull('deleted_at')
              query.preload("productVariation", (query) => {
            ...
            treatments: treatments.map((elem) => ({
              ...
              items: elem.items.map((item) => ({
                ...
                departmentItems: departmentTreatmentItems
                  .filter((ro) => ro.treatment_id === elem.id && ro.treatment_item_id === item.id)
                  .map((r) => ({
                ...
                executions: item.executions
                  .filter((ex) => ex.treatment_item_id === item.id && ex.treatment_id === elem.id)
                  .map((itemExec) => ({
                    ...
                    user: this.sharedService.captureGroup(itemExec.executionUser, (v) => ({
                      id: v.id,
                      name: v.name,
                    })),
  ...
  public async show(unitId: string, id: string) {
    ...
    const departmentItemRows = await Database.from("budget_items")
      ...
      .whereRaw("budget_items.budget_id = ? and budget_items.business_unit_id = ?", [
        result.id,
        unitId,
      ]);
    ...
    jsonData.items = jsonData.items.map((bi: BudgetItem) => {
      ...
      bi.departmentItems = departmentItemRows.filter(
        (ro: { product_variation_id: string; budget_item_id: string }) => {
          return bi.id === ro.budget_item_id || bi.product_variation_id === ro.product_variation_id;
        },
      );
  ...
  public async searchProducts(unitId: string, data: ISearchProduct) {
    ...
    const qb = Database.from("products")
      ...
      .joinRaw(
        "join product_variations pv on products.id = pv.product_id and pv.deleted_at is null",
      )
      .joinRaw(
    ...
    return [...result, ...kits.map((elem) => ({ ...elem.toJSON(), type: "kit" }))];
  }
  ...
  public async createBudget(authCtx: AuthContext, data: ICreateBudgetData) {
    return Database.transaction(async (trx) => {
      ...
      if (authCtx.unit.unitConfig.reviewer === "O" && !data.reviewerId) {
        throw new BadRequestException("É necessário informar o avaliador", 400, "E_ERR");
      }
      ...
      const budget = await Budget.create(
        {
          ...
          tag: GenerateTag(Number.parseInt(authCtx.unit.unitConfig.budgetCounter, 10) + 1),
        },
        {
          client: trx,
        },
      ...
      for (const item of data.items) {
        const variation = items.find((variation) => variation.id === item.productVariationId);

        if (!variation) {
          throw this.sharedService.ResourceNotFound("Variação do produto não encontrada");
        }
        ...
        const bi = await budget.related("items").create(
          {
            ...
            totalValue: item.courtesy ? 0 : item.quantity * item.unitaryValue - item.discountValue,
            status: BudgetStatus.A,
          },
          {
            client: trx,
  ...
  public async updateBudget(
    ...
    return Database.transaction(async (trx) => {
      ...
      const tasks = data.items.map(async (elem) => {
        ...
        if (elem.departmentId && elem.departmentItemId && elem.budgetItemDepartmentId) {
          await BudgetItemDepartment.query()
            .useTransaction(trx)
            .where("id", elem.budgetItemDepartmentId)
            .where("budget_id", budget.id)
            .where("budget_item_id", elem.budgetItemId ? elem.budgetItemId : v4())
  ...
  public async createBudgetItem(authCtx: AuthContext, data: ICreateBudgetItemData) {
    return Database.transaction(async (trx) => {
      ...
      if (
        ...
        throw new BadRequestException("Desconto máximo foi excedido", 400, "E_MAX_DISCOUNT");
      }
      ...
      const item = await budget.related("items").create(
        {
          ...
          totalValue: data.courtesy ? 0 : data.quantity * data.unitaryValue - data.discountValue,
          status: BudgetStatus.A,
        },
        {
          client: trx,
      ...
      await budget
        .merge({
          ...
          productValue:
            productVariation.product.type === ProductType.PRODUCT && !data.courtesy
              ? budget.productValue + item.totalValue
              : budget.productValue,
          serviceValue:
            productVariation.product.type === ProductType.SERVICE && !data.courtesy
  ...
  public async createBudgetItems(authCtx: AuthContext, data: ICreateBudgetItemData[]) {
    return Database.transaction(async (trx) => {
      const result = await this.sharedService.checkDiscount(
        trx,
        authCtx,
      ...
      const tasks = data.map(async (item) => {
        ...
        if (!variation) {
          throw new BadRequestException("Variação não encontrada", 400, "E_ERR");
        }

        await budget
          .merge({
            pending: data.some((f) => f.courtesy || f.maxDiscount) ? true : budget.pending,
            productValue:
  ...
  public async updateBudgetItem(authCtx: AuthContext, data: IUpdateBudgetItemData[]) {
    return Database.transaction(async (trx) => {
      ...
      const result = await this.sharedService.checkDiscount(
        ...
        data.map((elem) => ({
          variationId:
            budgetItems.find((f) => f.id === elem.budgetItemId)?.product_variation_id ?? v4(),
          unitaryValue: elem.unitaryValue,
          discountValue: elem.discountValue,
          quantity: elem.quantity,
          courtesy: elem.courtesy,
      ...
      const tasks = data.map(async (elem) => {
        ...
        if (!budgetItem) {
          throw new BadRequestException("Item do orçamento não encontrado", 400, "E_ERR");
        }
  ...
  public async deleteBudgetItem(authCtx: AuthContext, id: string) {
    return Database.transaction(async (trx) => {
      ...
      if (budgetItem.budget.paidValue > budgetItem.budget.totalValue - budgetItem.totalValue) {
        throw new BadRequestException(
  ...
  public async confirmBudget(authCtx: AuthContext, id: string, data: IConfirmBudgetData) {
    ...
    return Database.transaction(async (trx) => {
      ...
      await Patient.query()
        .useTransaction(trx)
        .whereIn("id", [model.client_id, model.patient_id].filter(Boolean) as string[])
        .update({
      ...
      const createdBillTasks = items
        ...
        .map(async (item) => {
          const rule = rules.find(
            (r) => r.taxation_group_id === item.productVariation.product.taxation_group_id,
          );
          const totalValue = item.quantity
            .times(item.unitaryValue)
            .minus(item.discountValue)
            .toNumber();
          const icmsBase = rule ? totalValue * ((100 - rule.icmsPercRedBaseCalculo) / 100) : 0;
          const icmsStBase = rule ? icmsBase + (icmsBase * rule.ivaIcmsSt) / 100 : 0;
          const icmsValue = rule
            ? icmsBase *
              ((rule.icmsPercRedBaseCalculo * ((100 - rule.icmsPercRedAliquota) / 100)) / 100)
            : 0;
          ...
          const bi = await BillItem.create(
            {
              ...
              icmsStBase:
                item.productVariation.product.type === ProductType.PRODUCT ? icmsStBase : undefined,
              ...
              issCst:
                item.productVariation.product.type === ProductType.SERVICE
                  ...
                  : undefined,
              issBase:
                item.productVariation.product.type === ProductType.SERVICE ? totalValue : undefined,
              issPercentage:
                item.productVariation.product.type === ProductType.SERVICE
                  ? rule?.icmsPerc
                  : undefined,
          ...
          const deptoItem = existingDepartmentItems.find((r) => r.budget_item_id === item.id);
          if (deptoItem) {
            await BillItemDepartment.create(
              {
                bill_id: bill.id,
      ...
      const tasks = itemsToUpdate.map((elem) => {
        return elem
          .merge({
            status: data.notConfirmedItems.includes(elem.id) ? BudgetStatus.N : BudgetStatus.C,
          })
          .useTransaction(trx)
          .save();
      });
      ...
      await model
        .merge({
          ...
          observation: data.observation,
          cancelation_reason_id: data.type === "PARCIAL" ? data.reasonId : undefined,
          canceledObservation: data.type === "PARCIAL" ? data.canceledObservation : undefined,
          internalObservation: [model.internalObservation, data.internalObservation]
            .filter(Boolean)
            .join(" - "),
          productValue: totalProductValue,
          serviceValue: totalServiceValue,
  ...
  public async cancelBudget(unitId: string, id: string, user: User, data: ICancelBudgetData) {
    const model = await Budget.query().where("id", id).where("business_unit_id", unitId).first();
  ...
  public async deleteBudget(authCtx: AuthContext, id: string) {
    return Database.transaction(async (trx) => {
      ...
      if (model.status !== BudgetStatus.A) {
        throw new BadRequestException("Orçamento com status inválido para inclusão", 400, "E_ERR");
      }
  ...
  public async addFromKit(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (budget.status !== BudgetStatus.A) {
        throw new BadRequestException("Orçamento não está aberto", 400, "E_ERR");
      }
      ...
      await budget
        .merge({
          productValue: budget.productValue + items.reduce((acc, item) => acc + item.totalValue, 0),
          discountValue:
            budget.discountValue + items.reduce((acc, item) => acc + item.discountValue, 0),
          totalValue: budget.totalValue + items.reduce((acc, item) => acc + item.totalValue, 0),
        })
        .useTransaction(trx)
        .save();
    });
  ...
  public async createBudgetPayments(
    ...
    return Database.transaction(async (trx) => {
      ...
      const paymentMethodFlags = await PaymentMethodFlag.query()
        .useTransaction(trx)
        .whereIn("id", data.items.map((d) => d.paymentMethodFlagId ?? 0).filter(Boolean));
      ...
      const tasks = data.items.map(async (elem, index) => {
        if (elem.paymentMethodFlagId) {
          ...
          if (!paymentMethodFlag) {
            throw new InternalErrorException("Bandeira não encontrada", 500, "E_ERR");
          }
          ...
          if (
            elem.installments > (paymentMethodFlag.installmentsWithoutPassword ?? 0) &&
            !elem.maxParcelas
          ) {
            throw new BadRequestException(
          ...
          const paymentMethod = paymentMethods.find((pm) => pm.id === elem.paymentMethodId);
          if (!paymentMethod) {
            throw new InternalErrorException("Método não encontrado", 500, "E_ERR");
          }
          ...
          if (
            elem.installments > (paymentMethod.installmentsWithoutPassword ?? 0) &&
            !elem.maxParcelas
          ) {
            throw new BadRequestException(
              "Este Orçamento ficará pendente de liberação pois o Numero de Parcelas lançado exige liberação. Deseja enviar para aprovação ?",
  ...
  public async updateBudgetPayment(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (row.status !== "Aberto") {
        throw new BadRequestException("Pagamento não está aberto", 400, "E_ERR");
      }
      ...
      const sum = this.sharedService.sum(budgetPayments.map((elem) => elem.totalValue));
      if (sum > row.budget.totalValue) {
        throw new BadRequestException("Valor pago do orçamento acima do total", 400, "E_ERR");
      }
  ...
  public async excludeBudgetPayment(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (data.origin === "Orçamento" && row.budget.status !== BudgetStatus.A) {
        throw new BadRequestException("Orçamento precisa estar Aberto", 400, "E_ERR");
      }
      ...
      await row.budget
        .merge({
          paidValue: this.sharedService.sum(budgetPayments.map((elem) => elem.totalValue)),
  ...
  public async listBudgetPayments(
    ...
    const qb = Database.from("budget_payments")
      ...
      .join("payment_methods", "payment_methods.id", "budget_payments.payment_method_id")
      .leftJoin("tef_flags", "tef_flags.id", "budget_payments.tef_flag_id")
      .leftJoin("tef_acquirers", "tef_acquirers.id", "budget_payments.tef_acquirer_id")
      .joinRaw(
      ...
      .joinRaw("left join users update_user on update_user.id = budget_payments.change_user_id")
      .joinRaw(
  ...
  async approveCourtesyOrMaxDiscount(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (!hasPermissions) {
        throw new BadRequestException("Usuário sem permissão de fazer a operação", 400, "E_ERR");
      }

      if (
        budget.items.some(
          (i) => i.status === BudgetStatus.A && (i.maxDiscount || i.courtesy) && !i.approved,
        )
      ) {
      ...
      if (budget.payments.some((p) => p.pending) && data.paymentsIdList.length === 0) {
        throw new BadRequestException(
