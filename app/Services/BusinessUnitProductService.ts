...
export default class BusinessUnitProductService {
  ...
  public async show(_: string, id: string) {
    const product = await BusinessUnitProduct.query().andWhere("id", id).first();
  ...
  public async store(unitId: string, data: IBusinessUnitProductData) {
    const product = await this.productService.show(unitId, data.productVariationId);
  ...
  public async update(unitId: string, id: string, data: IBusinessUnitProductData) {
