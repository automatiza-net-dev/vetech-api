...
export default class ClientOriginService {
  ...
  public async index(authCtx: AuthContext, search: ISearch) {
    ...
    if (search.existingOriginID) {
      query.whereRaw("(id = ? or deleted_at is null)", [search.existingOriginID]);
    } else {
  ...
  public async show(authCtx: AuthContext, id: string) {
    ...
    if (client.economic_group_id && client.economic_group_id !== authCtx.group.id) {
      throw this.sharedService.ResourceNotFound();
    }
  ...
  public async store(authCtx: AuthContext, data: Omit<IClientOriginData, "active">) {
  ...
  public async update(authCtx: AuthContext, id: string, data: IClientOriginData) {
