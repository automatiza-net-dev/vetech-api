  ...
  public async forPatient(authCtx: AuthContext, patientID: string) {
    ...
    return {
      ...
      contracts: contracts.map((row) => ({
        ...
        payment_method_tef_acquirer_description: row.tefAcquirer?.description ?? null,
      ...
      await Finance.createMany(
        newPayments.map<Partial<Finance>>((r) => {
          ...
          return {
            ...
            value: installmentValue - (installmentValue * r.paymentMethodDiscountPercentage) / 100,
            totalValue:
              installmentValue - (installmentValue * r.paymentMethodDiscountPercentage) / 100,
            feeDiscountValue:
  ...
  public async update(
    ...
    return Database.transaction(async (trx) => {
      const existingContract = await PatientContract.query()
        ...
        .first();
      if (!existingContract) {
        throw new ResourceNotFoundException("Contrato não encontrado", 404, "E_ERR");
      }
