...
export default class CrmV2Service {
  ...
  public async transferKanban(
    ...
    await Database.transaction(async (trx) => {
      ...
      await opp.merge({ kanban_id: data.destinationKanbanId }).useTransaction(trx).save();
    });
  }
  ...
  public async updateKanban(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (!kanban) {
        throw new ResourceNotFoundException("Kanban não encontrado", 404, "E_ERR");
      }
      ...
      await KanbanUser.query().useTransaction(trx).whereIn("id", toDelete).delete();
      ...
      const tasks = data.crmStatuses.map((row) => {
        if (row.id) {
          return CrmStatus.query().useTransaction(trx).where("id", row.id).update({
            user_updated_id: authCtx.user.id,
            description: row.description,
            ganho: row.ganhoStatus,
            perda: row.perdaStatus,
  ...
  public async searchKanbanOpportunities(
    ...
    for (const op of result) {
      const key = ["Faltou", "Desmarcou"].includes(op.status?.description ?? "Não específicado")
        ? "Faltou-Desmarcou"
  ...
  public async searchOpportunities(
    ...
    if (!data.kanban) {
      throw new BadRequestException("Parametro `kanban` é necessário", 400, "E_ERR");
    }
