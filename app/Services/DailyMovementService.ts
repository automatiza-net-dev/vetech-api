...
export default class DailyMovementService {
  ...
  async closeDailyMovement(authCtx: AuthContext, id: string, data: ICloseDailyMovementData) {
    ...
    if (!dailyMovement) {
      throw this.sharedService.ResourceNotFound("Movimento diário não encontrado");
    }
    ...
    const cashiers = await dailyMovement.related("cashiers").query().preload("entries");

    const hasOpenCashier = cashiers.some((cashier) => cashier.status === DailyCashierStatus.A);
    if (hasOpenCashier) {
      throw new BadRequestException("Existe um caixa aberto", 400, "E_DAILY_CASHIER_OPENED");
    }

    const expenses = cashiers
      .map((cashier) => cashier.entries.filter((entry) => entry.type === DailyCashierEntryType.D))
      .flat();

    const receipts = cashiers
      .map((cashier) => cashier.entries.filter((entry) => entry.type === DailyCashierEntryType.C))
      .flat();
    ...
    return dailyMovement
      .merge({
        ...
        expensesTotal: expenses.reduce(
          (total, expense) => total + parseFloat(expense.value as unknown as string),
          0,
        ),
        receiptsTotal: receipts.reduce(
          (total, receipt) => total + parseFloat(receipt.value as unknown as string),
          0,
        ),
      })
      .save();
  ...
  async checkDailyMovement(authCtx: AuthContext, id: string, data: ICheckedDailyMovementData) {
    const dailyMovement = await DailyMovement.query()
      .where("id", id)
      .where("business_unit_id", authCtx.unit.id)
      .first();
