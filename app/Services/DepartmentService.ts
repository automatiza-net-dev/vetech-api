...
export default class DepartmentService {
  async resume(authCtx: AuthContext, data: { type?: string; id?: string; description?: string }) {
  ...
  async index(
    ...
    const s3Urls = await SharedService.ComputePublicS3Link(
      [
        ...result.map((r) => r.image, ...result.flatMap((r) => r.items.map((r2) => r2.photo))),
  ...
  async update(
    ...
    if (!model) {
      throw new ResourceNotFoundException("Departamento não encontrado", 400, "E_ERR");
    }
  ...
  async listDepartmentProducts(
    ...
    if (!data.departmentId) {
      throw new BadRequestException("É preciso informar o departamento", 400, "E_ERR");
    }
    ...
    const qb = Database.from("departments")
      ...
      .joinRaw("left join products on department_products.product_id = products.id")
      .whereRaw(
  ...
  async listDepartmentItems(
    ...
    if (data.description) {
      qb.whereRaw("department_items.description ilike ?", [`%${data.description}%`]);
    }
...
json_agg(json_build_object('id', deptItems_temp.id, 'description', deptItems_temp.description, 'photo', deptItems_temp.photo, 'requiresObservation', deptItems_temp.requires_observation, 'order', deptItems_temp.order )) as items`),
      ...
      .whereRaw("(departments.economic_group_id = ? or departments.economic_group_id is null)", [
        authCtx.group.id,
      ])
      .whereRaw("(departments.business_unit_id = ? or departments.business_unit_id is null)", [
        authCtx.unit.id,
      ])
...
order by products.description ) deptProd_temp  on departments.id = deptProd_temp.department_id`,
    ...
    const s3Urls = await SharedService.ComputePublicS3Link(
      [...items.map((r) => r.image), ...items.flatMap((r) => r.items.map((r2) => r2.photo))].filter(
        Boolean,
      ),
    );
  ...
  async storeDepartmentProducts(
    ...
    if (!authCtx.hasPermission("DPT01")) {
      throw new UnauthorizedException("Usuário sem permissão para fazer a operação", 400, "E_ERR");
    }
  ...
  async updateDepartmentItem(
    ...
    return await Database.transaction(async (trx) => {
      ...
      const photoKey = data.photo ? `${v4()}.${data.photo.extname}` : item.photo;
      if (data.photo) {
        await data.photo.moveToDisk(
          "department-items",
          {
