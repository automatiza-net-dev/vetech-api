...
export default class DepositService {
  ...
  public async updateDepositItem(
    ...
    return Database.transaction(async (trx) => {
      const row = await DepositItem.query()
        ...
        .whereHas("deposit", (q) => {
          q.where("economic_group_id", authCtx.group.id).where("business_unit_id", authCtx.unit.id);
        })
        .first();
  ...
  public async createDepositMovement(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (!fromRow) {
        throw new BadRequestException("Deposito de estoque não encontrado", 400, "E_NOT_FOUND");
      }
      ...
      if (fromRow.type !== "Ajuste" && data.items.length > 0) {
        const result1 = await this.$checkDepositItems(trx, fromRow, data.items, "Origem");
        if (result1.length !== 0) {
          return result1;
        }
...
where deposit_id = ?
  ...
  public async validateDepositOperation(
    ...
    const result = await Database.rawQuery(`select * from check_quantity_on_deposit(?, ?, ?)`, [
      authCtx.user.id,
      authCtx.unit.id,
      JSON.stringify(
        data.map((elem) => ({
