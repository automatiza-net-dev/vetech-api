...
export default class DreGroupService {
  public async index(authCtx: AuthContext, data: { period?: string }) {
    ...
    const result: SqlResult[] = await Database.from("account_plans")
      ...
      .joinRaw("left join business_units on business_units.id = ?", [authCtx.unit.id])
      .joinRaw(
      ...
      .joinRaw("left join account_plans apPai on account_plans.parent_id = apPai.id", [])
      .joinRaw(
      ...
      .whereRaw("(account_plans.business_unit_id is null or account_plans.business_unit_id = ?)", [
        authCtx.group.id,
      ])
      .orderByRaw(
    ...
    return Object.keys(groups).reduce((currArray, groupKey) => {
      ...
      currArray.push({
        id: b_id,
        identification: result.find((r) => r.b_id === b_id)?.identification ?? "Não encontrado",
        periodo,
        agrupamentos: uniqueDreGroups.map((group) => ({
          id: group,
          identification:
            result.find((r) => r.dg_id === group)?.agrupamento_dre ?? "Não encontrado",
          sequencia: result.find((r) => r.dg_id === group)?.sequence ?? 0,
          grupo_plano_contas: result
            .filter(
              (gpContas) =>
                gpContas.b_id === b_id && gpContas.periodo === periodo && gpContas.dg_id === group,
            )
            .reduce((currArray, gpContas) => {
              if (!currArray.find((c) => c.id === gpContas.id_grupo_plano_contas)) {
                currArray.push({
                  ...
                  grupo_planos_contas_pai: result
                    ...
                    .reduce((currArray, gpContasPai) => {
                      if (!currArray.find((c) => c.id === gpContasPai.id_plano_contas_pai)) {
                        currArray.push({
                          ...
                          grupo_planos_contas: result
                            .filter(
                              (gpContasFilho) =>
                                gpContasPai.id_plano_contas_pai === gpContasFilho.id_plano_contas,
                            )
                            .reduce((currArray, gpContasFilho) => {
                              if (!currArray.find((c) => c.id === gpContasFilho.id_plano_contas)) {
                                currArray.push({
  ...
  public async store(
    ...
    try {
      ...
      if (
        ...
        "constraint" in e &&
        // @ts-ignore
        e.constraint.includes("dre_groups_economic_group_id_system_id_sequence_unique")
      ) {
  ...
  public async storePlanning(
    ...
    try {
      ...
      if (contraint?.includes("dre_cost_plannings_business_unit_id_period_unique")) {
  ...
  public async updatePlanning(
    ...
    try {
      await Database.transaction(async (trx) => {
        ...
        if (!planning) {
          throw new BadRequestException("Planejamento não encontrado", 400, "E_ERR");
        }
