...
export default class DreService {
  ...
  public async generateDreSpreadsheet(
    ...
    const data = await Database.from("finances")
      ...
      .joinRaw("join economic_groups on finances.economic_group_id = economic_groups.id")
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        data.map((d) => ({
          ...d,
          mes: Number.parseInt(d.mes, 10),
          valor_pago: Number.parseFloat(d.valor_pago),
          valor_recebido: Number.parseInt(d.valor_recebido, 10),
    ...
    if (!result.success) {
      throw new InternalErrorException(`Erro gerando pdf -> ${result.err}`, 500, "E_ERR");
    }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (responseJobData.status === "completed") {
        const exportTask = responseJobData.tasks.find((t) => t.name === "export-1");
        if (exportTask) {
