...
import FinanceReversal, { FinanceReversalType } from "App/Models/FinanceReversal";
...
export default class FinanceService {
  ...
  async calculateFees(
    ...
    const tefFlag = await TefFlag.query().where("id", data.tefFlagId).firstOrFail();
    ...
    if (!installment) {
      throw new BadRequestException("Parcela não permitida para essa bandeira", 400, "BAD_REQUEST");
    }
  ...
  async index(unitId: string, data: ISearch) {
    ...
    if (data.fromExpirationDate) {
      financesQb.whereRaw("expiration_date::date >= ?", [data.fromExpirationDate]);
    }

    if (data.toExpirationDate) {
      financesQb.whereRaw("expiration_date::date <= ?", [data.toExpirationDate]);
    }
  ...
  async reducedIndex(
    ...
    const qb = Database.from("finances")
      ...
      .joinRaw("left join account_plans on finances.account_plan_id = account_plans.id", [])
      .joinRaw("left join payment_methods on finances.payment_method_id = payment_methods.id", [])
    ...
    if (data.fromExpirationDate) {
      qb.whereRaw("finances.expiration_date::date >= ?", [data.fromExpirationDate]);
    }

    if (data.toExpirationDate) {
      qb.whereRaw("finances.expiration_date::date <= ?", [data.toExpirationDate]);
    }
    ...
    if (!data.order || data.order === "expiration_date") {
      qb.orderByRaw("ordem, expiration_date, document, installment, issue_date");
    }

    if (data.order === "issue_date") {
      qb.orderByRaw("ordem, issue_date,  document, installment , expiration_date");
    }

    if (data.order === "payment_date") {
      qb.orderByRaw("ordem, payment_date, document, installment, issue_date, expiration_date");
    }

    if (data.order === "competence_date") {
      qb.orderByRaw("ordem, competence_date, document, installment, issue_date, expiration_date");
    }

    if (data.order === "doc") {
      qb.orderByRaw("ordem, document, installment, issue_date, expiration_date, client");
    }
    ...
    if (data?.groupBorderos === "sim") {
      ...
      qb.union((builder) => {
        ...
        account_plans.description                                               as account_plan
          ...
          .joinRaw("left join account_plans on borderos.account_plan_id = account_plans.id", [])
          .joinRaw(
            "left join payment_methods on borderos.payment_method_id = payment_methods.id",
            [],
          )
          .joinRaw("left join tef_flags on borderos.tef_flag_id = tef_flags.id", [])
        ...
        if (data.fromIssueDate) {
          builder.whereRaw("borderos.issue_date::date >= ?", [data.fromIssueDate]);
        }
        ...
        if (data.fromExpirationDate) {
          builder.whereRaw("borderos.expiration_date::date >= ?", [data.fromExpirationDate]);
        }

        if (data.toExpirationDate) {
          builder.whereRaw("borderos.expiration_date::date <= ?", [data.toExpirationDate]);
        }

        if (data.fromPaymentDate) {
          builder.whereRaw("borderos.payment_date::date >= ?", [data.fromPaymentDate]);
        }

        if (data.toPaymentDate) {
          builder.whereRaw("borderos.payment_date::date <= ?", [data.toPaymentDate]);
        }
        ...
        if (data.status === FinanceStatus.A) {
          builder.whereIn("borderos.status", ["Aberto", "Fechado"] as TBorderoStatus[]);
        }
    ...
    qb.union((builder) => {
      builder
       ...
       payment_methods.checking_account_id as payment_methods_checking_account_id
        ...
        .joinRaw("left join patients on borderos.client_id = patients.id", [])
        .joinRaw("left join payment_methods on borderos.payment_method_id = payment_methods.id", [])
        .joinRaw("left join tef_flags on borderos.tef_flag_id = tef_flags.id", [])
        .whereIn("borderos.business_unit_id", units)
        .whereNull("borderos.deleted_at")
 ...
 tef_acquirers.id as tef_adquirente_id, payment_methods.checking_account_id as payment_methods_checking_account_id`),
        ...
        .joinRaw("join payment_methods on finances.payment_method_id = payment_methods.id", [])
        .joinRaw("left join tef_flags on finances.tef_flag_id = tef_flags.id", [])
        .joinRaw("left join tef_acquirers on finances.acquirer_id = tef_acquirers.id", [])
        .whereNull("finances.deleted_at")
        .whereIn("finances.business_unit_id", units)
 ...
 payment_methods.type, coalesce(tef_acquirers.description, 'Adquirente Não Informada'), tef_acquirers.id, payment_methods.checking_account_id `,
      ...
      if (data.fromIssueDate) {
        builder.whereRaw("finances.issue_date::date >= ?", [data.fromIssueDate]);
      }
      ...
      if (data.fromExpirationDate) {
        builder.whereRaw("finances.expiration_date::date >= ?", [data.fromExpirationDate]);
      }

      if (data.toExpirationDate) {
        builder.whereRaw("finances.expiration_date::date <= ?", [data.toExpirationDate]);
      }

      if (data.fromPaymentDate) {
        builder.whereRaw("finances.payment_date::date >= ?", [data.fromPaymentDate]);
      }

      if (data.toPaymentDate) {
        builder.whereRaw("finances.payment_date::date <= ?", [data.toPaymentDate]);
      }
      ...
      if (data.internalCode) {
        builder.whereRaw("finances.internal_code ilike ?", [`%${data.internalCode}%`]);
      }
      ...
      if (data.fiscalNote) {
        builder.whereRaw("finances.fiscal_note ilike ?", [`%${data.fiscalNote}%`]);
      }
    });
...
tef_flags.description as tef_flag, payment_methods.tef as pm_tef, payment_methods.type as pm_type, coalesce(tef_acquirers.description, 'Adquirente Não Informada') as tef_adquirente `),
  ...
  async createFinance(authCtx: AuthContext, data: IUpsertFinance) {
    ...
    return await Database.transaction(async (trx) => {
      ...
      const dailyCashier = await this.sharedService.getContextCashier(authCtx, trx);
      ...
      return Finance.create(
        {
          ...
          paymentValue: data.paymentValue ? new Decimal(data.paymentValue) : null,
          ...
          additionValue: data.increaseValue ? new Decimal(data.increaseValue) : undefined,
  ...
  async createMultipleFinances(authCtx: AuthContext, data: IUpsertFinance[]) {
    if (authCtx.unit.unitConfig.requiresFinanceClient && data.some((item) => !item.clientId)) {
      throw new BadRequestException(
    ...
    await Database.transaction(async (trx) => {
      const tasks = data.map(async (item) => {
        const paymentMethod = await PaymentMethod.findOrFail(item.paymentMethodId, {
          client: trx,
        });
        const dailyMovement = await DailyMovement.query()
          .useTransaction(trx)
        ...
        return Finance.create(
          {
            ...
            paymentValue: item.paymentValue ? new Decimal(item.paymentValue) : undefined,
            ...
            additionValue: item.increaseValue ? new Decimal(item.increaseValue) : undefined,
  ...
  // 2.2
  async updateFinance(unitId: string, _: User, id: string, data: IUpdateFinance) {
    return Database.transaction(async (trx) => {
      ...
      const paymentMethod = await PaymentMethod.findOrFail(data.paymentMethodId, { client: trx });
  ...
  async updateFinanceDown(authCtx: AuthContext, data: { items: IFinanceDownData[] }) {
    return Database.transaction(async (trx) => {
      for await (const elem of data.items) {
        ...
        finance.merge({
          ...
          additionValue: elem.increaseValue ? new Decimal(elem.increaseValue) : undefined,
        ...
        const banking = await Banking.create(
          {
            ...
            balance:
              finance.type === FinanceType.C
                ? (checkingAccount?.balance ?? 0) + (finance.paymentValue?.toNumber() ?? 0)
                : (checkingAccount?.balance ?? 0) - (finance.paymentValue?.toNumber() ?? 0),
        ...
        await checkingAccount
          .merge({
            balance:
              finance.type === FinanceType.C
                ? checkingAccount.balance + (finance.paymentValue?.toNumber() ?? 0)
                : checkingAccount.balance - (finance.paymentValue?.toNumber() ?? 0),
          })
          .useTransaction(trx)
          .save();
  ...
  // 2.7
  async updateFinanceReversal(unitId: string, id: string, data: IFinanceReversalData) {
    ...
    return Database.transaction(async (trx) => {
      ...
      const checkingAccount = await CheckingAccount.find(finance.checking_account_id);
  ...
  async deleteMultipleFinances(
    ...
    await Database.transaction(async (trx) => {
      const accessResult: { control_id: string; erro: string }[] = await Database.from(
        "role_permissions",
      )
        .select(
          Database.raw(
        ...
        .useTransaction(trx)
        .joinRaw("join permissions p on role_permissions.permission_id = p.id")
        .joinRaw(
      ...
      if (finances.length !== data.idList.length) {
        throw new BadRequestException("Algum registro enviado não foi encontrado", 400, "E_ERR");
      }
      ...
      const badOriginFinances = finances.filter((f) => f.originFlag !== FinanceOriginFlag.F);
      ...
      return Finance.query().useTransaction(trx).whereIn("id", data.idList).update({
        exclusion_user_id: authCtx.user.id,
        status: FinanceStatus.E,
...
case when p.control_id = 'TRC11' then 'Usuário não possui permissão para retirar o aceito de titulos de Credito' else 'Usuário não possui permissão para para retirar o aceito de titulos de Debito' end as erro`,
  ...
  async getExpiringExpenses(authCtx: AuthContext) {
    const hasPermission = await this.sharedService.userHasPermission(authCtx, "PRI03");
    if (!hasPermission) {
      throw new UnauthorizedException("Usuário sem permissão", 400, "E_ERR");
    }
    ...
    return Array.from(dataSet.keys()).map((elem) => ({
      supplier: finances.find((e) => e.client_id === elem)?.client?.name ?? "Título sem fornecedor",
  ...
  async getExpiringPayments(authCtx: AuthContext) {
    ...
    return finances.map((elem) => ({
      ...
      supplier: {
        ...
        accountPlan: this.sharedService.captureGroup(elem.client?.tutor?.accountPlan, (v) => ({
          id: v.id,
  ...
  async getControlResumeOptional(
    ...
    if (!data.checkingAccountId || !data.iterationDateFrom || !data.iterationDateTo) {
      return {
        saldoinicial: "-",
        saldofinal: "-",
        saldoconta: "-",
...
and (fSaldo.payment_date::date <= now()::date or fSaldo.expiration_date::date <= now()::date)
      ...
      .joinRaw("join checking_accounts ca on finances.checking_account_id = ca.id")
    ...
    if (data.internalCode) {
      rowsQb.whereRaw("finances.internal_code ilike ?", [`%${data.internalCode}%`]);
    }
    ...
    if (data.fromExpirationDate) {
      rowsQb.whereRaw("finances.expiration_date::date >= ?", [data.fromExpirationDate]);
    }

    if (data.toExpirationDate) {
      rowsQb.whereRaw("finances.expiration_date::date <= ?", [data.toExpirationDate]);
    }

    if (data.fromPaymentDate) {
      rowsQb.whereRaw("finances.payment_date::date >= ?", [data.fromPaymentDate]);
    }
    ...
    const rowsQb = Database.from("titulos")
      ...
      .with("titulos", (cteQb) => {
       ...
       (payment_date is null and expiration_date::date between ? and ?))`,
        ...
        if (data.fromIssueDate) {
          cteQb.whereRaw("finances.issue_date::date >= ?", [data.fromIssueDate]);
        }
        ...
        if (data.internalCode) {
          cteQb.whereRaw("finances.internal_code ilike ?", [`%${data.internalCode}%`]);
        }
        ...
        if (data.fromExpirationDate) {
          cteQb.whereRaw("finances.expiration_date::date >= ?", [data.fromExpirationDate]);
        }

        if (data.toExpirationDate) {
          cteQb.whereRaw("finances.expiration_date::date <= ?", [data.toExpirationDate]);
        }

        if (data.fromPaymentDate) {
          cteQb.whereRaw("finances.payment_date::date >= ?", [data.fromPaymentDate]);
        }

        if (data.toPaymentDate) {
          cteQb.whereRaw("finances.payment_date::date <= ?", [data.toPaymentDate]);
        }
       ...
       (payment_date is null and expiration_date::date between ? and ?))`,
          ...
          if (data.fromIssueDate) {
            unionQb.whereRaw("borderos.issue_date::date >= ?", [data.fromIssueDate]);
          }

          if (data.toIssueDate) {
            unionQb.whereRaw("borderos.issue_date::date <= ?", [data.toIssueDate]);
          }

          if (data.internalCode) {
            unionQb.whereRaw("borderos.internal_code ilike ?", [`%${data.internalCode}%`]);
          }

          if (data.historic) {
            unionQb.whereRaw("borderos.historic ilike ?", [`%${data.historic}%`]);
          }

          if (data.fromExpirationDate) {
            unionQb.whereRaw("borderos.expiration_date::date >= ?", [data.fromExpirationDate]);
          }

          if (data.toExpirationDate) {
            unionQb.whereRaw("borderos.expiration_date::date <= ?", [data.toExpirationDate]);
          }

          if (data.fromPaymentDate) {
            unionQb.whereRaw("borderos.payment_date::date >= ?", [data.fromPaymentDate]);
          }

          if (data.toPaymentDate) {
            unionQb.whereRaw("borderos.payment_date::date <= ?", [data.toPaymentDate]);
          }
          ...
          if (data.checkingAccountId) {
            unionQb.where("borderos.checking_account_id", data.checkingAccountId);
          }
        });
      });
  ...
  async getOpenDailyCashiers(authCtx: AuthContext) {
    const hasPermission = await this.sharedService.userHasPermission(authCtx, "PRI04");
    if (!hasPermission) {
      throw new UnauthorizedException("Usuário sem permissão", 400, "E_ERR");
    }
  ...
  async getOpenAttendances(authCtx: AuthContext) {
    ...
    return result.map((elem) => ({
      ...
      scheduleService: this.sharedService.captureGroup(elem.scheduleService, (v) => ({
        id: v.id,
        description: v.description,
      })),
    }));
    ...
    return Database.transaction(async (trx) => {
      ...
      const updatedBordero = await bordero
        .merge({
          ...
          paymentValue: bordero.totalValue + data.interestValue - data.discountValue,
  ...
  async calculateFee(finance: Finance, paymentMethod: PaymentMethod, tefFlagID: string) {
    const paymentMethodFlag = await PaymentMethodFlag.query()
      .where("payment_method_id", paymentMethod.id)
  ...
  public async dashboardFinanceResume(authCtx: AuthContext) {
    ...
    const [expenses, payments, overallResume, checkingResume] = await Promise.all([
    ...
    return [
      ...
      },
      {
        ...
        data: payments.map((elem) => ({
          description: elem.paymentMethod.description,
          value: this.sharedService.formatter.format(elem.totalValue.toNumber()),
        })),
      },
      {
        name: "overall-resume",
  ...
  private async $registerBankingDown(trx: TransactionClientContract, bordero: Bordero) {
    const checkingAccount = await CheckingAccount.find(bordero.checking_account_id, {
      client: trx,
    });

    const relativeValue = bordero.type === "Debito" ? -bordero.paymentValue : bordero.paymentValue;
    ...
    await bordero.merge({ bank_statement_id: banking.id }).useTransaction(trx).save();
  ...
  private async $revertRegisterBankingDown(trx: TransactionClientContract, bordero: Bordero) {
    const checkingAccount = await CheckingAccount.find(bordero.checking_account_id, {
      client: trx,
    });
  ...
  private async $syncBordero(trx: TransactionClientContract, bordero: Bordero) {
    const finances = await bordero.related("finances").query().useTransaction(trx);

    const totalValueSum = finances.reduce((acc, curr) => acc + curr.totalValue.toNumber(), 0);

    await bordero
      .merge({
        borderoValue: totalValueSum,
        totalValue: totalValueSum + bordero.interestValue - bordero.discountValue,
