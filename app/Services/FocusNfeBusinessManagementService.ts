  ...
  public async createBusiness(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (businessUnit.focusRef) {
        throw new BadRequestException("Esta unidade já está cadastrada no FocusNFe", 400, "E_ERR");
      }
      ...
      if (data.models.includes(0)) {
        ...
        payload.proximo_numero_nfse_producao = +dynamicData.proximoNumeroNfseProducao;
      ...
      if (data.models.includes(65)) {
        payload.proximo_numero_nfce_producao = +dynamicData.proximoNumeroNfceProducao;
        ...
        payload.arquivo_certificado_base64 = dynamicData.arquivoCertificadoBase64.toString();
      ...
      if (data.models.includes(55)) {
        payload.proximo_numero_nfe_producao = +dynamicData.proximo_numero_nfe_producao;
      ...
      try {
        ...
        await businessUnit.merge({ focusRef: data.id.toString() }).useTransaction(trx).save();
        ...
        if (error instanceof AxiosError) {
          ...
          if (!error.response?.data.mensagem) {
            throw new BadRequestException("Erro na comunicação com a FocusNFe", 400, "E_ERR");
          }

          if (!Array.isArray(error.response.data.erros)) {
            throw new BadRequestException(error?.response?.data.mensagem, 400, "E_ERR");
          }

          throw new BadRequestException(
            error.response.data.erros
              .reduce(
                (acc: string[], curr: { codigo: string; mensagem: string; campo: null }) => {
