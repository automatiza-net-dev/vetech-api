...
export default class FocusNfeService {
  ...
  public async sendNfe(ref: string, rawPayload: ISendNfe, token: string) {
    const payload = {
      ...
      data_entrada_saida: rawPayload.model === "65" ? undefined : rawPayload.authorizedAt,
      tipo_documento: "1",
      local_destino: "1", // doc
      ...
      complemento_emitente: rawPayload.seller.location.complement.substring(0, 30),
      ...
      logradouro_destinatario:
        rawPayload.model === "65" ? undefined : rawPayload.buyer.location.street,
      numero_destinatario: rawPayload.model === "65" ? undefined : rawPayload.buyer.location.number,
      complemento_destinatario:
        rawPayload.model === "65"
          ? undefined
          : rawPayload.buyer.location.complement?.substring(0, 30),
      bairro_destinatario:
        rawPayload.model === "65" ? undefined : rawPayload.buyer.location.district,
      municipio_destinatario:
        rawPayload.model === "65" ? undefined : rawPayload.buyer.location.city,
      uf_destinatario: rawPayload.model === "65" ? undefined : rawPayload.buyer.location.uf,
      cep_destinatario: rawPayload.model === "65" ? undefined : rawPayload.buyer.location.code,
      telefone_destinatario: rawPayload.model === "65" ? undefined : rawPayload.buyer.phone,
    ...
    try {
      const { data } = await this.ax.post(
        rawPayload.model === "65" ? `/v2/nfce?ref=${ref}` : `/v2/nfe?ref=${ref}`,
  ...
  private mapToNationalNfsePayload(data: ISendNationalNfse) {
    ...
    const payload: Record<string, any> = {
      ...
      codigo_municipio_tomador:
        data.buyer.address.cityCode === 0 ? undefined : data.buyer.address.cityCode,
  ...
  public async getDownloadLinks(
    ...
    if (!record) {
      throw new ResourceNotFoundException("Arquivo não encontrado", 404, "E_BACKUP_NOT_FOUND");
    }
