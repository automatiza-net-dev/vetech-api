...
export default class HospitalizationMedicalPrescriptionService {
  ...
  public async schedulingIndex(unitId: string, data: ISearchScheduling) {
    const query = HospitalizationMedicalPrescriptionScheduling.query()
      .where("status", "Aberto" as THospitalizationMedicalPrescriptionSchedulingStatus)
      .whereHas("hospitalization", (query) => {
    ...
    if (data.fromScheduledDate && data.toScheduledDate) {
      ...
      query.whereRaw("(scheduled_at::date = ? and scheduled_at::time between ? and ?)", [
  ...
  public async store(
    ...
    return Database.transaction(async (trx) => {
      ...
      throw new BadRequestException("Combinação de tipo e frequência inválida", 400, "E_INVALID");
  ...
  public async update(
    ...
    if (["E", "I", "C"].includes(entity.status)) {
      throw new BadRequestException("Prescrição em estado inválido", 400, "E_INVALID");
    }
  ...
  public async interruptPrescription(authCtx: AuthContext, id: string) {
    await Database.transaction(async (trx) => {
      ...
      if (entity.status !== "Aberto") {
        throw new BadRequestException("Prescrição não está ativa", 400, "E_INVALID");
      }
      ...
      await entity.related("scheduling").query().useTransaction(trx).where("status", "A").update({
        status: "I",
        excludedAt: DateTime.now(),
        update_user_id: authCtx.user.id,
      });
  ...
  public async updateScheduling(
    ...
    await Database.transaction(async (trx) => {
      const scheduling = await HospitalizationMedicalPrescriptionScheduling.query()
        .useTransaction(trx)
        .where("id", id)
      ...
      if (
        scheduling.status !== ("Aberto" as THospitalizationMedicalPrescriptionSchedulingStatus) ||
      ...
      const prescriptionSchedulings = await HospitalizationMedicalPrescriptionScheduling.query()
        .useTransaction(trx)
        .where(
  ...
  public async delete(id: string) {
    const entity = await HospitalizationMedicalPrescription.query().where("id", id).first();
  ...
  async createScheduling(prescription: HospitalizationMedicalPrescription, user: User) {
    ...
    const offset =
      prescription.frequencyUnit === MedicalPrescriptionFrequencyUnit.DAY ? 60 * 24 : 60;
    ...
    const data: Array<Partial<HospitalizationMedicalPrescriptionScheduling>> = Array.from<
      Partial<HospitalizationMedicalPrescriptionScheduling>
    >({
  ...
  async createResume(prescription: HospitalizationMedicalPrescription) {
    ...
    if (prescription.type === MedicalPrescriptionType.MEDICATION) {
      return [
        ...
        [prescription?.dose, prescription?.prescriptionUnit?.name].filter(Boolean).join(" "),
  ...
  calculateEndDate(prescription: HospitalizationMedicalPrescription) {
    if (prescription.frequencyQuantityUnit === MedicalPrescriptionFrequencyQuantityUnit.DAY) {
      return prescription.prescribedAt.plus({ days: prescription.frequencyQuantity }).toJSDate();
    }

    if (prescription.frequencyQuantityUnit === MedicalPrescriptionFrequencyQuantityUnit.HOUR) {
      return prescription.prescribedAt.plus({ hours: prescription.frequencyQuantity }).toJSDate();
    }
