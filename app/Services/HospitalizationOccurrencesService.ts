...
import Occurrence, { OccurrenceType, OccurrenceTypeLabels } from "App/Models/Occurrence";
...
export default class HospitalizationOccurrencesService {
  ...
  public async store(authCtx: AuthContext, data: Omit<IHospitalizationOccurrenceData, "active">) {
    return Database.transaction(async (trx) => {
      ...
      const ent = await hospitalization.related("occurrences").create(
        {
          ...
          hospitalization_medical_prescription_id: data.hospitalizationMedicalPrescriptionId,
      ...
      if (data.attachments) {
        const attachments = await Promise.all(data.attachments.map(this.uploadFile));
      ...
      if (occurrence.type === OccurrenceType.ALTA_INTERNACAO) {
        ...
        await HospitalizationTimeline.create({
          ...
          data: {
            ...
            type: HospitalizationType[hospitalization.type],
            releaseType: data.resume.includes("AO") ? "Alta Indicada" : "Alta Sem Indicação",
            description: data.description,
            resume: data.resume,
          },
        });
  ...
  public async storeAttachments(unitId: string, data: IHospitalizationOccurrenceAttachmentData) {
  ...
  public async update(unitId: string, id: string, data: IHospitalizationOccurrenceData) {
    const hospitalization = await this.getHospitalization(unitId, data.hospitalizationId);

    const ent = await hospitalization.related("occurrences").query().where("id", id).first();
  ...
  public async delete(authCtx: AuthContext, id: string) {
    await Database.transaction(async (trx) => {
      ...
      if (
        [OccurrenceType.OCORRENCIA, OccurrenceType.RELATORIO_MEDICO].includes(ent.occurrence.type)
      ) {
        await HospitalizationTimeline.updateMany(
          {
            "meta.occurrence": ent.id,
  ...
  public async deleteAttachment(unitId: string, id: string, attachment: string) {
    ...
    const attachmentEnt = await ent.related("attachments").query().where("id", attachment).first();
