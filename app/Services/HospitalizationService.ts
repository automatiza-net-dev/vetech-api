...
export default class HospitalizationService {
  ...
  public async parsedIndex(unitId: string, data: IHome) {
    ...
    if (data.tutorName) {
      qb.whereHas("tutor", (query) => {
        query.whereRaw("name ~* ?", [data.tutorName ?? ""]).where("type", PatientType.TUTOR);
      });
    }
  ...
  public async patientTimeline(id: string) {
    const patient = await Patient.query().where("id", id).preload("hospitalizations").first();
  ...
  public async index(unitId: string, data: ISearch) {
    ...
    qb.where("business_unit_id", unitId).where("status", HospitalizationStatus.ACTIVE);
  ...
  public async completedIndex(unitId: string, data: ISearch) {
    ...
    qb.where("business_unit_id", unitId).where("status", HospitalizationStatus.COMPLETE);
  ...
  public async getScheduling(unitId: string, id: string) {
    const qb = Hospitalization.query().where("business_unit_id", unitId).where("id", id);
  ...
  public async store(authCtx: AuthContext, data: IHospitalizationData) {
    const ent = await Database.transaction(async (trx) => {
      ...
      const technician = data.userId ? await User.findOrFail(data.userId) : authCtx.user;
      ...
      if (existingInternation) {
        throw new BadRequestException("Paciente já internado", 400, "E_ALREADY_HOSPITALIZED");
      }
      ...
      if (occurrence) {
        ...
        Logger.error("Não existe ocorrência de internação cadastrada para o grupo econômico");
      }
  ...
  public async update(authCtx: AuthContext, id: string, data: IHospitalizationData) {
    await Database.transaction(async (trx) => {
      ...
      if (!hospitalization || hospitalization.business_unit_id !== authCtx.unit.id) {
        throw this.sharedService.ResourceNotFound();
      }
      ...
      const updatedHospitalization = await hospitalization
        .merge({
          ...
          releasedAt: data.status === HospitalizationStatus.COMPLETE ? DateTime.now() : undefined,
        })
        .useTransaction(trx)
        .save();
      ...
      if (data.status === HospitalizationStatus.COMPLETE) {
        const timelineInfo = await TimelineType.findOrFail(hospitalizationTimeline.id, {
          client: trx,
        });
  ...
  public async completeHospitalization(unitId: string, id: string, user: User) {
    ...
    await Database.transaction(async (trx) => {
      ...
      if (hospitalization.status === HospitalizationStatus.COMPLETE) {
        throw new BadRequestException("Internação já finalizada", 400, "E_CLOSED_ALREADY");
      }
      ...
      await HospitalizationMedicalPrescriptionScheduling.query()
        ...
        .update({
          status: "Interrompido" as THospitalizationMedicalPrescriptionSchedulingStatus,
        });
      ...
      patient: {
        ...
        info: hospitalization.patient.patientAnimal
          ? {
              ...
              specie: hospitalization.patient.patientAnimal.race.specie.description,
              hair: hospitalization.patient.patientAnimal?.hair?.description ?? null,
              age: hospitalization.patient.birthDate
                ? DateTime.now()
                    .diff(DateTime.fromJSDate(hospitalization.patient.birthDate), "years")
                    .toObject().years
                : null,
