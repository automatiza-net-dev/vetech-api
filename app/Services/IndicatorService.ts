...
import { TripleFunnelSvg } from "App/Assets/Svg";
...
export default class IndicatorService {
  ...
  public async medianTicketByOrigin_2(
    ...
    return {
      ...
      legend: result.map((elem, idx) => [
        ...
        {
          title: "Tkt Medio R$",
          value: this.shared.formatter.format(elem.total / Number.parseInt(elem.qty_clients)),
          itemStyle: { color: "" },
        },
      ]),
        ...
        series: [
          {
            ...
            data: result.map((elem, idx) => ({
              ...
              percentage: Number.parseFloat(((elem.total / sum) * 100).toFixed(2)),
              itemStyle: {
                color: authCtx.group.colors[idx % authCtx.group.colors.length],
              },
            })),
  ...
  public async invoicingByProductType(
    ...
    const qb = Database.from("bill_items")
      ...
      .leftJoin("product_variations", (query) => {
        query.on("product_variations.id", "=", "bill_items.product_variation_id");
      })
      ...
      .leftJoin("business_unit_products", (query) => {
        query
          .on("business_unit_products.product_variation_id", "=", "bill_items.product_variation_id")
          .andOn("business_unit_products.businness_unit_id", "=", "bill_items.business_unit_id");
      })
      .leftJoin("business_units", (query) => {
        query.on("business_units.id", "=", "bills.business_unit_id");
      })
  ...
  public async invoicingByProductType_2(
    ...
    return {
      ...
      legend: result.map((elem, idx) => ({
        ...
        percentage: this.shared.formatPercentage((elem.total_sales / parsedTotal) * 100),
        name: elem.description,
        itemStyle: {
          color: authCtx.group.colors[idx % authCtx.group.colors.length],
        },
        ...
        series: [
          {
            ...
            data: result.map((elem, idx) => ({
              ...
              percentage: Number.parseFloat(((elem.total_sales / parsedTotal) * 100).toFixed(2)),
              name: elem.description,
              itemStyle: {
                color: authCtx.group.colors[idx % authCtx.group.colors.length],
              },
  ...
  public async invoicingByProductTypeWithSubgroup(
    ...
    if (!data.subgroup) {
      throw new BadRequestException("Subgrupo não informado", 400, "BAD_REQUEST");
    }

    const listOfUnits = data.units && Array.isArray(data.units) ? data.units : [authCtx.unit.id];
    ...
    const qb = Database.from("bill_items")
      ...
      .groupBy("products.id", "products.description", "business_units.id", "subgroups.description")
      .whereNull("bills.deleted_at")
    ...
    const sum = result.reduce((acc, curr) => acc + Number.parseFloat(curr.total_sales), 0);
  ...
  public async invoicingByPaymentMethod_2(
    ...
    return {
      ...
      legend: result.map((elem, idx) => [
        ...
        {
          title: "Partic %",
          value: this.shared.formatPercentage((elem.totalpayments / total) * 100),
          itemStyle: { color: "" },
        },
        {
          title: "Total R$",
        ...
        series: [
          {
            ...
            data: result.map((elem, idx) => ({
              ...
              name: elem.description,
              percentage: Number.parseFloat(((elem.totalpayments / total) * 100).toFixed(2)),
              itemStyle: {
                color: authCtx.group.colors[idx % authCtx.group.colors.length],
              },
            })),
  ...
  public async schedulingIndicators(
    ...
    const opportunityLogsQb = Database.from("opportunity_logs")
      ...
      .joinRaw("join business_units on opportunity_logs.business_unit_id = business_units.id", [])
      .joinRaw("join crm_statuses on opportunity_logs.status_id = crm_statuses.id", [])
    ...
    if (data.fromDate && data.toDate) {
      qb.andWhereRaw("schedules.start_hour::date between ? and ?", [data.fromDate, data.toDate]);
      salesQb.andWhereRaw("bills.bill_date::date between ? and ?", [data.fromDate, data.toDate]);
      opportunityLogsQb.andWhereRaw("opportunity_logs.contact_date::date between ? and ?", [
        data.fromDate,
        data.toDate,
      ]);
    }
    ...
    return generalResult.map((elem) => ({
      ...
      opportunities: Number.parseInt(
        opportunityResult.find((r) => elem.id === r.id)?.novas_oportunidades ?? "0",
      ),
      scheduled: Number.parseInt(elem.agendados, 10),
      attended: Number.parseInt(elem.atendidos, 10),
      sales: Number.parseInt(salesResult.find((r) => r.id === elem.id)?.sales ?? "0"),
      clients: Number.parseInt(salesResult.find((r) => r.id === elem.id)?.clients ?? "0"),
    }));
  }
  ...
  public async subgroupIndicators(
    ...
    const qb = Database.from("bills")
      ...
      .join("product_variations", "product_variations.id", "bill_items.product_variation_id")
  ...
  public async subgroupTreeIndicators(
    ...
    const qb = Database.from("bills")
      ...
      .havingRaw("sum(bill_items.quantity) > 0 and sum(bill_items.total_value) > 0", [])
      .orderByRaw("2, 6")
      .whereNull("bills.deleted_at");
      ...
      return {
        ...
        children: result
          ...
          .map((elem) => ({
            ...
            percentage: this.shared.formatPercentage((elem.total / $total) * 100),
          })),
      };
    });
  }
  ...
  public async opportunitiesIndicators(
    ...
    if (data.fromDate) {
      qb.andWhereRaw("opportunity_logs.contact_date::date >= ?", [data.fromDate]);
    }
  ...
  public async crmIndicators(
    ...
    if (data.fromDate && data.toDate) {
      opportunityQb.andWhereRaw("opportunity_logs.contact_date::date between ? and ?", [
        data.fromDate,
        data.toDate,
      ]);
    }
  ...
  public async crmIndicators_2(
    ...
    const _novos = Number.parseInt(result.at(0)?.novas_oportunidades ?? "0", 10);
    const _agendados = Number.parseInt(result.at(0)?.agendados ?? "0", 10);
    const _comparecidos = Number.parseInt(result.at(0)?.comparecidos ?? "0", 10);
  ...
  public async projectionIndicators(
    ...
    const qb = Database.from("bills")
      ...
      .joinRaw("join economic_groups on business_units.economic_group_id = economic_groups.id", [])
      .groupBy(
        "economic_groups.id",
  ...
  > {
    const dt = DateTime.fromISO(
      data.fromDate ? new Date(data.fromDate).toISOString() : new Date().toISOString(),
    ).plus({ days: 10 });

    const ym = dt.toFormat("yyyyMM");

    const usefulDays = authCtx.unit.unitConfig.crmUsefulDays
      ? differenceInBusinessDays(endOfMonth(dt.toJSDate()), startOfMonth(dt.toJSDate())) + 1
      : (dt.daysInMonth ?? 30);
    ...
    if (data.fromDate && data.toDate) {
      qb.andWhereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
  ...
  public async budgetsIndicators(
    ...
    if (!data.type) {
      throw new BadRequestException("Informe o tipo de orçamento", 400, "E_ERR");
    }
    ...
    const qb = Database.from("budgets")
      ...
      .joinRaw("join business_units on budgets.business_unit_id = business_units.id")
      .joinRaw("join economic_groups on business_units.economic_group_id = economic_groups.id")
    ...
    return result.map((elem) => {
      return {
        ...
        avgOpenValue:
          elem.open === "0" ? 0 : elem.total_open_value / Number.parseInt(elem.open, 10),
      };
    });
  }
  ...
  public async budgetsByStatusIndicators(
    ...
    if (!data.status) {
      throw new BadRequestException("Informe o status de orçamento", 400, "E_ERR");
    }
  ...
  public async marketingIndicators(
    ...
    const financesQb = Database.from("finances")
      ...
      .joinRaw("join business_units on finances.business_unit_id = business_units.id")
      .joinRaw("join economic_groups on business_units.economic_group_id = economic_groups.id")
      .joinRaw(
    ...
    const [billsResult, financesResult] = await Promise.all([billsQb, financesQb]);
    ...
    for (const finance of financesResult) {
      const key = [finance.e_id, finance.b_id, finance.competence_date].join("__");
      keys.add(key);
    }
    ...
    return Array.from(keys.keys())
      .map((key) => {
        ...
        const bill = billsResult.find(
          (bill) =>
            bill.e_id === e_id && bill.b_id === b_id && bill.competence_date === competence_date,
        );
        ...
        return {
          group: {
            id: e_id,
            name: [bill, finance].filter(Boolean).at(0)?.company_name ?? "Não encontrado?",
          },
          unit: {
            id: b_id,
            name: [bill, finance].filter(Boolean).at(0)?.identification ?? "Não encontrado?",
          },
          competenceDate: competence_date,
          totalFinance: financeTotal,
          totalBills: billTotal,
          roi: financeTotal === 0 ? 0 : (billTotal - financeTotal) / financeTotal,
        };
      })
      .sort((a, b) => {
  ...
  public async costOfAcquisitionIndicators(
    ...
    const billsQb = Database.from("bills")
      ...
      .joinRaw("left join business_units on bills.business_unit_id = business_units.id")
      .joinRaw("left join economic_groups on business_units.economic_group_id = economic_groups.id")
      .joinRaw(
      ...
      .joinRaw(`join client_origins co on pt.client_origin_id = co.id and co.group = 'Marketing'`)
      .whereNull("bills.deleted_at")
      .groupByRaw(
        `economic_groups.id, business_units.id, to_char(bill_date, 'MM/yyyy'), business_units.id, economic_groups.id`,
      );
  ...
  public async billPaymentFormatIndicators(
    ...
    const qb = Database.from("bills")
      ...
      .joinRaw("join payment_methods on bill_payments.payment_method_id = payment_methods.id")
      .groupByRaw(
  ...
  public async avgReceiptDeadlineIndicators(
    ...
    const qb = Database.from("bills")
      ...
      .joinRaw("join payment_methods on bill_payments.payment_method_id = payment_methods.id")
      .groupByRaw(`economic_groups.id, business_units.id, to_char(bills.bill_date, 'YYYY/MM')`)
      .whereNull("bills.deleted_at")
      .whereRaw(
        `((to_char(bills.bill_date, 'YYYY/MM') <> to_char(bill_payments.expiration_date, 'YYYY/MM'))
    or (payment_methods.tef <> 'NAO' and payment_methods.type = 'CREDITO'))`,
  ...
  public async clientGroupTreeIndicators(
    ...
    const qb1 = Database.from("bills")
      ...
      .joinRaw("inner join business_units on business_units.id = bills.business_unit_id")
      .groupByRaw("business_units.id")
      .whereNull("bills.deleted_at")
      .whereRaw(`to_char(bills.bill_date, 'YYYY-MM') <> to_char(patients.first_sale, 'YYYY-MM')`);
    ...
    const total = result.reduce((acc, curr) => acc + Number.parseFloat(curr.total), 0);
      ...
      acc.push({
        ...
        grupos: categoryGroups.map((elem) => {
          ...
          return {
            ...
            porcentagem: this.shared.formatPercentage((groupTotal / categorySum) * 100),
            origem_clientes: result
              .filter((r) => r.categoria === curr)
              .filter((r) => r.grupo === elem)
              .map((ori) => ({
                origem: ori.description,
                total: this.shared.formatter.format(Number.parseFloat(ori.total)),
  ...
  public async clientOriginTreeIndicators(
    ...
    const qb = Database.from("opportunities")
      ...
      .joinRaw("left join client_origins on client_origins.id = opportunities.client_origin_id")
      .joinRaw(
      ...
      .joinRaw("join business_units on opportunities.business_unit_id = business_units.id")
      .groupByRaw(
    ...
    const categories = keys.reduce((acc, curr) => {
      ...
      const categorySum = categoryRows.reduce((sumAcc, sumCurr) => sumAcc + sumCurr.count, 0);
      ...
      acc.push({
        categoria: curr,
        total: categorySum,
        porcentagem: this.shared.formatPercentage((categorySum / totalCount) * 100),
        grupos: categoryGroups.map((elem) => {
          const groupTotal =
            elem === "-"
              ? result
          ...
          return {
            ...
            origem_clientes: result
              ...
              .map((ori) => ({
                origem: ori.origem,
                total: ori.count,
                porcentagem: this.shared.formatPercentage((ori.count / groupTotal) * 100),
              })),
          };
        }),
      });
  ...
  public async sanclaChartsIndicators(authCtx: AuthContext, data: Record<string, any>) {
    ...
    const billing = cards.at(0) as Awaited<ReturnType<typeof this.billingIndicators>>;
    const medianTicket = cards.at(1) as Awaited<ReturnType<typeof this.medianTicket>>;
    const openBudgets = cards.at(2) as Awaited<ReturnType<typeof this.budgetsByStatusIndicators>>;
    const cancelledBudgets = cards.at(3) as Awaited<
      ReturnType<typeof this.budgetsByStatusIndicators>
    >;
    const marketing = cards.at(4) as Awaited<ReturnType<typeof this.marketingIndicators>>;
    const cac = cards.at(5) as Awaited<ReturnType<typeof this.costOfAcquisitionIndicators>>;
    const installmentAvg = cards.at(6) as Awaited<ReturnType<typeof this.installmentAvgIndicators>>;
              ...
              items: [
                {
                  ...
                  value: this.shared.formatter.format(
                    cards[0] ? cards[0].reduce((acc, curr) => acc + curr.total, 0) : 0,
                  ),
                },
              ],
            }
              ...
              items: [
                {
                  ...
                  value: this.shared.formatter.format(
                    cards[0] ? (cards[0].reduce((acc, curr) => acc + curr.meta.value, 0) ?? 0) : 0,
                  ),
                },
              ],
            }
              ...
              items: [
                {
                  ...
                  value: this.shared.formatPercentage(
                    cards[0] ? (cards[0].reduce((acc, curr) => acc + curr.percentage, 0) ?? 0) : 0,
                  ),
                },
              ],
            }
              ...
              items: [
                {
                  ...
                  percentage: this.shared.formatPercentage(
                    cards[0] ? cards[0].reduce((acc, curr) => acc + curr.metaProjection, 0) : 0,
                  ),
                  value: this.shared.formatter.format(
                    cards[0] ? cards[0].reduce((acc, curr) => acc + curr.projection, 0) : 0,
                  ),
                  color: authCtx.hasPermission("IND14") ? (billing.at(0)?.color ?? "#000") : "#000",
                },
              ],
            }
          : null,
              ...
              items: [
                {
                  ...
                  value: this.shared.formatter.format(
                    medianTicket ? medianTicket.salesTotal / medianTicket.qtyClients : 0,
                  ),
                },
              ],
            }
              ...
              items: [
                {
                  ...
                  value: (marketing.reduce((acc, curr) => acc + curr.roi, 0) ?? 0).toFixed(2),
                },
              ],
            }
          : null,
  ...
  public async liftOneChartsIndicators(authCtx: AuthContext, data: Record<string, any>) {
    const charts = await Promise.all([
      SharedService.NoopPromise(
        () => authCtx.hasPermission("IND01"),
        () => this.invoicingByPaymentMethod_2(authCtx, data),
    ...
    const medianTicket = cards.at(0) as Awaited<ReturnType<typeof this.medianTicket>>;
    const billPaymentFormat = cards.at(1) as Awaited<
      ReturnType<typeof this.billPaymentFormatIndicators>
    >;
    const installmentAvg = cards.at(2) as Awaited<ReturnType<typeof this.installmentAvgIndicators>>;
    const treeIndicators = cards.at(6) as Awaited<
      ReturnType<typeof this.clientGroupTreeIndicators>
    >;
    const subgroupTree = cards.at(4) as Awaited<ReturnType<typeof this.subgroupTreeIndicators>>;
        ...
        authCtx.hasPermission("IND10")
          ? {
              ...
              faturamento_realizado: {
                description: "Faturamento Realizado",
                value: this.shared.formatter.format(medianTicket?.salesTotal ?? 0),
              },
              items: [
                {
                  description: "Vendas a vista",
                  value: `${this.shared.formatPercentage(
                    (billPaymentCashSum / (billPaymentCashSum + billPaymentInstallmentSum)) * 100,
                  )} de Vendas a Vista`,
                },
                {
                  description: "Parcelamento Medio",
                ...
                },
                {
                  ...
                  value: `${this.shared.formatter.format(
                    (medianTicket?.salesTotal ?? 0) / (medianTicket?.qtyClients ?? 1),
  ...
  public async chartsIndicators(authCtx: AuthContext, data: Record<string, any>) {
    const hasPermission = authCtx.hasPermission("PRI05");
    if (!hasPermission) {
      throw new UnauthorizedException("Usuário sem permissão para ver os gráficos", 400, "E_ERR");
    }
  ...
  public async invoicingNewClientsPeriod_2(
    ...
    if (data.fromDate && data.toDate) {
      qb.whereRaw(`bill_date::date between (?::date - interval '5 months')::date and ?`, [
        data.fromDate,
        data.toDate,
      ]);
    }
      ...
      legend: [
        [
          ...
          },
          {
            title: "Partic %",
            value: this.shared.formatPercentage(
              result.map((r) => (r.a_vista / (aVistaSum + aPrazoSum)) * 100).at(0) ?? 0,
            ),
            itemStyle: { color: "" },
          },
          {
            title: "Total R$",
            value: this.shared.formatter.format(result.map((r) => r.a_vista).at(0) ?? 0),
            itemStyle: { color: "" },
          },
          {
            title: "Qtd Cli",
        ...
        [
          ...
          },
          {
            title: "Partic %",
            value: this.shared.formatPercentage(
              result.map((r) => (r.a_prazo / (aVistaSum + aPrazoSum)) * 100).at(0) ?? 0,
            ),
            itemStyle: { color: "" },
          },
          {
            title: "Total R$",
            value: this.shared.formatter.format(result.map((r) => r.a_prazo).at(0) ?? 0),
            itemStyle: { color: "" },
          },
          {
            title: "Qtd Cli",
        ...
        series: [
          {
            ...
            valor: result.map((r) => (r.a_vista / (aVistaSum + aPrazoSum)) * 100),
          },
          {
            name: "A Prazo",
            type: "bar",
            ...
            valor: result.map((r) => (r.a_prazo / (aVistaSum + aPrazoSum)) * 100),
          },
        ],
      },
    };
          ...
          .map((row) => ({
            ...
            morning: {
              ...
              avg:
                row.manha_total === 0
                  ? this.shared.formatter.format(0)
                  : this.shared.formatter.format(row.manha_total / Number.parseInt(row.manha_qtd)),
            },
            afternoon: {
              ...
              avg:
                row.tarde_total === 0
                  ? this.shared.formatter.format(0)
                  : this.shared.formatter.format(row.tarde_total / Number.parseInt(row.tarde_qtd)),
            },
            night: {
              ...
              avg:
                row.noite_total === 0
                  ? this.shared.formatter.format(0)
                  : this.shared.formatter.format(row.noite_total / Number.parseInt(row.noite_qtd)),
            },
          }));
      }),
    };
  ...
  public async productTypeIndicators_2(
    ...
    const metasResult = await qb;
    const productSum = metasResult.reduce((acc, curr) => acc + curr.product_total, 0);
    const serviceSum = metasResult.reduce((acc, curr) => acc + curr.service_total, 0);
    ...
    return {
      ...
      legend: [
        [
          ...
          {
            title: "Partic %",
            value: this.shared.formatPercentage((productSum / (productSum + serviceSum)) * 100),
            itemStyle: { color: "" },
          },
          {
            title: "Total R$",
        ...
        [
          ...
          {
            title: "Partic %",
            value: this.shared.formatPercentage((serviceSum / (productSum + serviceSum)) * 100),
            itemStyle: { color: "" },
          },
          {
            title: "Total R$",
        ...
        return {
          ...
          subgroups: result
            ...
            .map((elem) => ({
              ...
              total: this.shared.formatter.format(elem.total),
              uniqueClients: Number.parseInt(elem.clients, 10),
              percentage: this.shared.formatPercentage((elem.total / parsedTotal) * 100),
            })),
        };
      }),
    };
  ...
  public async salesPerPeriodIndicators_2(
    ...
    return {
      ...
      data: uniqueUnits.map((elem) => {
        ...
        const sum = unit.madrugada_total + unit.manha_total + unit.tarde_total + unit.noite_total;
        ...
        return {
          ...
          period: {
            dawn: {
              ...
              recurrent: this.shared.formatter.format(unit.madrugada_recorrentes),
              percentage: this.shared.formatPercentage((unit.madrugada_total * 100) / sum),
            },
            morning: {
              ...
              percentage: this.shared.formatPercentage((unit.manha_total * 100) / sum),
            },
            afternoon: {
              ...
              percentage: this.shared.formatPercentage((unit.tarde_total * 100) / sum),
            },
            night: {
              ...
              percentage: this.shared.formatPercentage((unit.noite_total * 100) / sum),
            },
          },
        };
      }),
            ...
            return {
              ...
              users: result
                ...
                .map((elem) => ({
                  ...
                  confirmed: Number.parseInt(elem.confirmed, 10),
                  totalConfirmedValue: this.shared.formatter.format(elem.total_confirmed_value),
                  avgConfirmedValue:
                    elem.confirmed === "0"
                      ? this.shared.formatter.format(0)
                      : this.shared.formatter.format(
                  ...
                  cancelled: Number.parseInt(elem.cancelled, 10),
                  totalCancelledValue: this.shared.formatter.format(elem.total_cancelled_value),
                  avgCancelledValue:
                    elem.cancelled === "0"
                      ? this.shared.formatter.format(0)
                      : this.shared.formatter.format(
                  ...
                  open: Number.parseInt(elem.open, 10),
                  totalOpenValue: this.shared.formatter.format(elem.total_open_value),
                  avgOpenValue:
                    elem.open === "0"
                      ? this.shared.formatter.format(0)
                      : this.shared.formatter.format(
  ...
  public async consolidatedReviewerBudgets(
    ...
    if (data.fromDate && data.toDate) {
      qb.whereRaw("total.budget_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
          ...
          users: uniqueUsers.map((user) => {
            ...
            return {
              ...
              participacaoRealizado: this.shared.formatPercentage(
                (Number.parseFloat(userRow.total_confirmados) / confirmedSum) * 100,
              ),
              conversaoAvaliacoes: this.shared.formatPercentage(
                Number.parseFloat(userRow.conv_venda),
              ),
              qtdAvaliacoes: userRow.qtd_total,
              totalAvaliado: this.shared.formatter.format(userRow.total_orcamentos),
  ...
  public async schedulingIndicators_2(
    ...
    if (authCtx.system.type === "Vet") {
      const _agendados = Number.parseInt(generalResult.at(0)?.agendados ?? "0", 10);
      const _atendidos = Number.parseInt(generalResult.at(0)?.atendidos ?? "0", 10);
      const _vendidos = Number.parseInt(generalResult.at(0)?.vendidos ?? "0", 10);
  ...
  public async schedulingOpportunitiesIndicators_2(
    ...
    const _oportunidades = Number.parseInt(
      opportunityResult.find((r) => r.id === generalResult.at(0)?.id)?.novas_oportunidades ?? "0",
      10,
    );
      ...
      configs: [
        {
          ...
          value: result.length === 0 ? 0 : Number.parseInt(result.at(0).novas, 10),
          normal: { fill: "blue" },
        },
        {
          name: "Agendadas",
          value: result.length === 0 ? 0 : Number.parseInt(result.at(0).agendadas, 10),
          normal: { fill: "red" },
        },
        {
          name: "Comparecidos",
          value: result.length === 0 ? 0 : Number.parseInt(result.at(0).comparecidas, 10),
          normal: { fill: "orange" },
        },
        {
          name: "Ganho",
          value: result.length === 0 ? 0 : Number.parseInt(result.at(0).ganhos, 10),
          normal: { fill: "orange" },
        },
      ],
    };
  ...
  public async salesPerUserIndicators_2(
    ...
    if (data.fromDate && data.toDate) {
      qb.andWhereRaw("bills.bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
        ...
        return {
          ...
          users: result
            ...
            .map((usr) => ({
              id: usr.u_id,
              name: usr.name,
              total: this.shared.formatter.format(Number.parseFloat(usr.total_value)),
              qty: usr.total_bills,
              avg: this.shared.formatter.format(Number.parseFloat(usr.avg_value)),
              percentage: this.shared.formatPercentage(
  ...
  public async invoicingNewClients_2(
    ...
    if (data.fromDate && data.toDate) {
      qb.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
    ...
    const totalSum = result.reduce(
      (acc, curr) =>
        acc + Number.parseFloat(curr.total_recorrentes) + Number.parseFloat(curr.total_novos),
      0,
    );
    ...
    const newSum = result.reduce((acc, curr) => acc + Number.parseFloat(curr.total_novos), 0);
    ...
    return {
      ...
      legend: [
        [
          ...
          {
            title: "Partic %",
            value: this.shared.formatPercentage((recurringSum / totalSum) * 100),
            itemStyle: { color: "" },
          },
          {
            title: "Total R$",
            ...
            data: [
              {
                value: recurringSum,
                name: "Recorrentes",
                percentage: Number.parseFloat(((recurringSum / totalSum) * 100).toFixed(2)),
                itemStyle: {
                  color: authCtx.group.colors.at(0),
                },
              },
              {
                value: newSum,
                name: "Novos",
                percentage: Number.parseFloat(((newSum / totalSum) * 100).toFixed(2)),
                itemStyle: {
                  color: authCtx.group.colors.at(1),
                },
              },
  ...
  public async monthlyIdealFunnelIndicators(
    ...
    const { faturamento, tkt_medio, conv_vendas, conv_comparecimentos, conv_agendamentos } =
      await this.generateComplexFunnelData(
        authCtx.system.id,
        authCtx.unit,
        format(data.fromDate ? addDays(new Date(data.fromDate), 10) : new Date(), "MM/yyyy"),
      );
    ...
    return {
      ...
      message: level4 === 0 ? "Não existe meta definida para o período selecionado" : null,
      configs: `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="330" viewBox="0 0 400 330" fill="none">
        <g clip-path="url(#clip0_2003_2250)">
        <path d="M306.709 96.4708L329.519 38.0934C331.043 34.1976 328.161 30 323.97 30H5.91384C1.80112 30 -1.08071 34.071 0.30648 37.9375L21.2217 96.315C22.0716 98.6816 24.3185 100.259 26.8291 100.259H301.16C303.612 100.259 305.82 98.7595 306.709 96.4805V96.4708Z" fill="${authCtx.group.colors.at(
          0,
  ...
  public async monthlyPartialFunnelIndicators(
    ...
    const firstFromInput = data.fromDate ? addHours(new Date(data.fromDate), 12) : new Date();
    ...
    const param1 = authCtx.unit.unitConfig.config.crm?.crm_useful_days
      ? differenceInBusinessDays(endOfMonth(dt.toJSDate()), startOfMonth(dt.toJSDate()))
      : (dt.daysInMonth ?? 30);
  ...
  public async monthlyRealizedFunnelIndicators(
    ...
    const colors: {
      ...
      cor_camada_comparecidas: string | null;
      cor_camada_ganhos: string | null;
      cor_conversao_novas_agendadas: string | null;
      cor_conversao_agendadas_comparecidas: string | null;
      cor_conversao_comparecidas_ganho: string | null;
    }[] = await Database.from("opportunity_logs")
      .select(
        Database.raw(
          `(select prg.color
        from "metas"
                 join business_unit_metas on metas.id = business_unit_metas.meta_id
                 join performance_range_goals prg on metas.id = prg.meta_id
        where "metas"."system_id" = ?
          and metas.description ilike 'Qtd Leads Crm'
          and "business_unit_metas"."business_unit_id" = ?
          and "business_unit_metas"."period" = ?
...
-- calculos para colorir as setas de conversão e % do funil
       (select prg.color
        from "metas"
                 ...
                 join performance_range_goals prg on metas.id = prg.meta_id
        where "metas"."system_id" = ?
          and metas.description ilike '% Vendas Crm'
          and "business_unit_metas"."business_unit_id" = ?
          and "business_unit_metas"."period" = ?
          and (cast(count(distinct opportunity_logs.opportunity_id)
                    FILTER ( WHERE crm_statuses.type = 'OP' and crm_statuses.tag = 'A' ) as float)
                   / case
                         when count(distinct opportunity_logs.opportunity_id)
       ...
       (select prg.color
        ...
        where "metas"."system_id" = ?
          ...
          and "business_unit_metas"."period" = ?
          and (cast(count(distinct opportunity_logs.opportunity_id)
                    FILTER ( WHERE crm_statuses.type = 'OP' and crm_statuses.tag = 'C' ) as float)
                   / case
                         when count(distinct opportunity_logs.opportunity_id)
       ...
       (select prg.color
        ...
        where "metas"."system_id" = ?
          ...
          and "business_unit_metas"."period" = ?
          and (cast(count(distinct opportunity_logs.opportunity_id)
                    FILTER ( WHERE crm_statuses.type = 'OPR' and crm_statuses.tag = 'G' ) as float)
                   / case
                         when count(distinct opportunity_logs.opportunity_id)
    ...
    const svgData = {
      $block_color_1: colors.at(0)?.cor_camada_novas_oportunidades ?? authCtx.group.colors.at(0),
      $block_color_2: colors.at(0)?.cor_camada_agendadas ?? authCtx.group.colors.at(1),
      $block_color_3: colors.at(0)?.cor_camada_comparecidas ?? authCtx.group.colors.at(2),
      $block_color_4: colors.at(0)?.cor_camada_ganhos ?? authCtx.group.colors.at(3),
      $novas: _novos,
      $agendadas: _agendados,
      $comparecidas: _comparecidos,
      $vendas: _ganhos,
      $novas_perc: this.shared.formatPercentage((_agendados / _novos) * 100),
      $agendadas_perc: this.shared.formatPercentage((_comparecidos / _agendados) * 100),
      $comparecidas_perc: this.shared.formatPercentage((_ganhos / _comparecidos) * 100),
      $arrow_color_1: colors.at(0)?.cor_conversao_novas_agendadas ?? authCtx.group.colors.at(0),
      $arrow_color_2: colors.at(0)?.cor_conversao_comparecidas_ganho ?? authCtx.group.colors.at(1),
      $arrow_color_3: colors.at(0)?.cor_conversao_comparecidas_ganho ?? authCtx.group.colors.at(2),
    };
    ...
    return {
      ...
      configs: Object.keys(svgData).reduce((currSvg, currKey) => {
        return currSvg.replace(currKey, svgData[currKey]);
      }, TripleFunnelSvg),
    };
  }

  private async generateComplexFunnelData(systemID: number, unit: BusinessUnit, period: string) {
    const [[sql1], [sql2], [sql3], [sql4], [sql5]] = await Promise.all([
      // Faturamento
      Database.from("metas")
        .select(Database.raw("business_unit_metas.value::float as faturamento"))
        .joinRaw("join business_unit_metas on metas.id = business_unit_metas.meta_id")
      ...
      // Conversão Crm
      Database.from("metas")
        .select(Database.raw("business_unit_metas.value::float as conv_comparecimentos"))
        .joinRaw("join business_unit_metas on metas.id = business_unit_metas.meta_id")
        .where("metas.system_id", systemID)
      ...
      Database.from("metas")
        .select(Database.raw("business_unit_metas.value::float as conv_agendamentos"))
        .joinRaw("join business_unit_metas on metas.id = business_unit_metas.meta_id")
        .where("metas.system_id", systemID)
  ...
  public async activityIndicators_2(
    ...
    const qb1 = Database.from("opportunities").select(
      Database.raw(
        `
  ...
  public async salesPerReviewerIndicator_2(
    ...
    if (!authCtx.hasPermission("IND28")) {
      throw new UnauthorizedException("Usuário sem permissão para ver o gráfico", 400, "E_ERR");
    }
    ...
    return {
      ...
      data: uniqueUnits.map((elem) => {
        ...
        const realizedSum = unitUsers.reduce((sum, curr) => sum + curr.total_realizado, 0);
        const reviewedSum = unitUsers.reduce((sum, curr) => sum + curr.total_avaliacoes, 0);
        ...
        return {
          ...
          users: unitUsers.map((usr) => ({
            ...
            ticketMedioRealizado: this.shared.formatter.format(usr.tkt_medio_realizado),
            participacaoRealizado: this.shared.formatPercentage(
            ...
            ticketMedioAvaliacoes: this.shared.formatter.format(usr.tkt_medio_avaliacoes),
          })),
        };
      }),
    };
        ...
        series: [
          {
            ...
            data: result.map((elem, idx) => ({
              value: elem.count,
              name: elem.origem,
              percentage: Number.parseFloat(((elem.count / sum) * 100).toFixed(2)),
              itemStyle: {
                color: authCtx.group.colors[idx % authCtx.group.colors.length],
              },
            })),
  ...
  public async activitiesTable_2(
    ...
    const qb2 = Database.from("opportunities")
      ...
      .joinRaw("join business_units on opportunities.business_unit_id = business_units.id")
      .whereNull("opportunities.deleted_at")
      .groupByRaw("business_units.id, business_units.identification, activities.description")
