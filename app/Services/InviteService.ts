...
export default class InviteService {
    ...
    return Database.transaction(async (trx) => {
      const rows: {
       ...
       user_unit_roles.unit_id                 as id_unidade`),
        ...
        .joinRaw("left join users_economic_groups on users.id = users_economic_groups.user_id")
        .joinRaw(
        ...
        .whereRaw("users_economic_groups.economic_group_id = ?", [authCtx.group.id])
      ...
      if (rows.length === 0) {
        ...
        const systemUrl = await SystemUrl.query()
          ...
          .first();
        if (!systemUrl) {
          throw new InternalErrorException("Sistema sem url configurada", 500, "E_ERR");
        }
  ...
  public async show(id: string): Promise<Invite> {
    ...
    if (!invite) {
      throw new ResourceNotFoundException("Convite não existe", 404, "E_NOT_FOUND");
    }
  ...
  public async check(id: string) {
    ...
    if (!user) {
      throw new BadRequestException("Convite sem usuário", 400, "E_BAD_REQUEST");
    }
  ...
  public async resendInvite(authCtx: AuthContext, id: string) {
    ...
    if (!invite) {
      throw new ResourceNotFoundException("Convite não existe", 404, "E_NO_INVITE");
    }

    if (!invite.active) {
      throw new BadRequestException("Convite não está ativo", 400, "E_INVITE_NOT_ACTIVE");
    }
    ...
    // });
    const url = await SystemUrl.query().where("system_id", authCtx.system.id).first();
    if (!url) {
      throw new BadRequestException("Sistema sem url configurada", 400, "E_ERR");
    }
    ...
    await Mail.send((message) => {
      message
        ...
        .htmlView("emails/invite", {
          url: [url.url, url.url.endsWith("/") ? "" : "/", `invites?token=${invite.id}`].join(""),
  ...
  public async update(authCtx: AuthContext, id: string, data: IInviteData): Promise<Invite> {
    const invite = await this.show(id);
    if (!invite.active) {
      throw new UnauthorizedException("Convite não está mais ativo", 401, "E_NOT_AUTHORIZED");
    }
  ...
  public async acceptInvite(data: IAcceptInvite): Promise<void> {
    ...
    if (!invite.active) {
      throw new BadRequestException("Convite não está mais ativo", 400, "E_BAD_REQUEST");
    }
    ...
    await Database.transaction(async (trx) => {
      ...
      if (!user) {
        throw new BadRequestException("Usuário não encontrado", 400, "E_BAD_REQUEST");
      }

      if (user.email !== data.email || !(await Hash.verify(user.password, data.password))) {
  ...
  public async acceptInviteForNewUser(data: IAcceptInviteNewUser): Promise<void> {
    const invite = await this.show(data.id);
    ...
    return Database.transaction(async (trx) => {
      ...
      await user.related("economicGroups").attach([invite.businessUnit.economicGroupId], trx);
  ...
  public async destroy(id: string, user: User): Promise<void> {
    const invite = await this.show(id);
    const inviteBusinessUnit = await invite.related("businessUnit").query().firstOrFail();
  ...
  private userIsRelatedToBusinessUnit(
    ...
    const relatedBusinessUnit = userBusinessUnits.find((u) => u.id === businessUnit.id);

    if (!relatedBusinessUnit) {
      Logger.warn("Usuário não pode enviar convite");
      throw new UnauthorizedException("Ação não permitida", 401, "E_NOT_AUTHORIZED");
    }
  }

  private async userBusinessUnits(user: User): Promise<Array<BusinessUnit>> {
    const entities = await user.related("economicGroups").query().preload("businessUnits");
