...
export default class KitService {
    ...
    return result.map((elem) => {
      return {
        ...
        sum: {
          ...
          discountPrice: elem.items.reduce((acc, curr) => acc + curr.discountPrice, 0),
  ...
  public async update(unitId: string, id: string, data: IKitData) {
    ...
    const ent = await Kit.query().where("id", id).andWhere("economic_group_id", group.id).first();
  ...
  public async addItemToKit(unitId: string, data: IUpsertKitItemData) {
    return Database.transaction(async (trx) => {
      ...
      if (variation.businessUnitProducts.some((p) => p.maximumDiscountValue < data.discountPrice)) {
        throw new BadRequestException(
          `Desconto lançado é superior ao permitido - ${variation.product.description}`,
          400,
          "E_MAX_DISCOUNT",
        );
      }
      ...
      const originalPrice = variation.businessUnitProducts[0]?.price ?? 0;
  ...
  public async updateItemToKit(
    ...
    return Database.transaction(async (trx) => {
      ...
      const kitItem = await KitItem.query().useTransaction(trx).where("id", itemId).firstOrFail();
