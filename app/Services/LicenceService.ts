...
export default class LicenceService {
  public async addAdditionalTrial({ unit }: { unit: string }): Promise<void> {
    ...
    if (!trialPlan) {
      ...
      throw new InternalErrorException("Erro interno", 400, "E_INTERNAL_SERVER_ERROR");
    }
    ...
    if (!activeLicence) {
      throw new BadRequestException("Não existe licença ativa", 400, "E_NO_LICENCE");
    }
    ...
    if (isBefore(new Date(), activeLicence.expirationDate)) {
      throw new BadRequestException("Licença ainda não expirou", 400, "E_BAD_LICENCE");
    }
