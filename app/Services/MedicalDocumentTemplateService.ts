...
export default class MedicalDocumentTemplateService {
  ...
  public async show(authCtx: AuthContext, id: string) {
    const template = await MedicalDocumentTemplate.query().where("id", id).first();

    if (!template) {
      throw this.sharedService.ResourceNotFound();
    }

    if (template.economic_group_id && template.economic_group_id !== authCtx.group.id) {
      throw this.sharedService.ResourceNotFound();
    }
  ...
  public async store(authCtx: AuthContext, data: Omit<IMedicalDocumentTemplateData, "active">) {
  ...
  public async update(authCtx: AuthContext, id: string, data: IMedicalDocumentTemplateData) {
