...
export default class NotificationsService {
  public async listNotifications(authCtx: AuthContext) {
    const notifications = await Database.from("notifications")
      ...
      .whereRaw(
        "id not in (select notification_id from notification_users where user_id = ? and read_at is not null)",
        [authCtx.user.id],
      );
  ...
  public async readNotifications(authCtx: AuthContext, data: { notificationIds: number[] }) {
  ...
  public async createNotification(
    ...
    const imageKey = data.image ? `${new Date().getTime()}-${data.image.clientName}` : null;
    if (data.image) {
  ...
  public async updateNotification(
    ...
    if (!notification) {
      throw new ResourceNotFoundException("Notification não encontrado", 404, "E_NOT_FOUND");
    }
  ...
  public async fullNotifications(authCtx: AuthContext): Promise<{ data: LogicalNotification[] }> {
  ...
  public async undefinedRoles(authCtx: AuthContext): Promise<{ data: LogicalNotification[] }> {
  ...
  public async pendingBills(authCtx: AuthContext): Promise<{ data: LogicalNotification[] }> {
  ...
  public async pendingBudgets(authCtx: AuthContext): Promise<{ data: LogicalNotification[] }> {
      ...
      data: [
        {
          ...
          status: "",
          message: "Existem Solicitações de Cancelamento de Vendas que necessitam de Avaliação",
          createdAt: null,
          createdAtText: null,
          isRead: false,
  ...
  public async pendingBillPaymentEvaluations(
    ...
    const [{ count }]: { count: number }[] = await Database.from("bill_payments")
