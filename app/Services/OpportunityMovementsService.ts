...
export default class OpportunityMovementsService {
  public async index(
    ...
    if (!data.type) {
      throw new BadRequestException("É preciso informar um `type`", 400, "E_ERR");
    }
  ...
  public async searchFromClients(
    ...
    if (!data.client) {
      throw new BadRequestException("É preciso informar um `client`", 400, "E_ERR");
    }
  ...
  > {
    const qb = Database.from("budgets")
      ...
      .whereRaw("budgets.budget_date::date >= now()::date - interval '30 days'", [])
