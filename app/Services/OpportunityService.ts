...
import OpportunityActivity, { TOpportunityActivityStatus } from "App/Models/OpportunityActivity";
...
export default class OpportunityService {
    ...
    const result = await Opportunity.query()
      ...
      .preload("client", (query) => {
        ...
        query.preload("patientAnimal", (query) => {
          query.select("race_id", "death", "death_date", "microchip", "castrated");
  ...
  public async searchOpportunities(
    ...
    if (data.clientName) {
      qb.whereHas("client", (query) => {
        query
          .whereRaw("name ~* ?", [`(${data.clientName?.toLowerCase().split(" ").join("|")})`])
          .where("type", PatientType.ANIMAL);
      });
    }

    if (data.description) {
      qb.whereRaw("lower(unaccent(opportunities.description)) ~* lower(unaccent(?))", [
        data.description,
      ]);
    }
    ...
    if (data.balance && Array.isArray(data.balance)) {
      ...
      const sanitizedOptions = data.balance.filter((f) => ["Ganho", "Perda"].includes(f));

      if (hasEmAberto && sanitizedOptions.length > 0) {
        qb.whereRaw(
          `(balance = ANY('{${sanitizedOptions.join(",")}}') or closing_date is null)`,
          [],
        );
      }
  ...
  and (
    ...
    return result.map((elem) => ({
      ...
      client: this.sharedService.captureGroup(elem.opportunity?.client, (v) => ({
        id: v.id,
        name: v.name,
        cellphone: v?.tutor?.cellphone ?? null,
        telephone: v?.tutor?.telephone ?? null,
      })),
      contact: this.sharedService.captureGroup(elem.opportunity?.contact, (v) => ({
        id: v.id,
        name: v.name,
        cellphone: v?.tutor?.cellphone ?? null,
        telephone: v?.tutor?.telephone ?? null,
      ...
      openingUser: this.sharedService.captureGroup(elem.opportunity?.openingUser, (v) => ({
        id: v.id,
        name: v.name,
      })),
      closingUser: this.sharedService.captureGroup(elem.opportunity?.closingUser, (v) => ({
        id: v.id,
        name: v.name,
      })),
      exclusionUser: this.sharedService.captureGroup(elem.opportunity?.exclusionUser, (v) => ({
        id: v.id,
        name: v.name,
      })),
  ...
  public async searchCompleteKanbanOpportunities(
    ...
    const qb = Opportunity.query()
      ...
      .preload("client", (query) => {
        query.select("id", "name", "weight", "gender", "client_origin_item_description");
    ...
    if (data.clientName) {
      qb.whereHas("client", (query) => {
        query
          .whereRaw("name ilike ?", [`%${data.clientName!.replaceAll(" ", "%")}%`])
          .where("type", PatientType.ANIMAL);
      });
    }
    ...
    if (data.contactName) {
      qb.whereHas("contact", (query) => {
        if (data.contactName) {
          query.whereRaw("name ilike ?", [`%${data.contactName!.replaceAll(" ", "%")}%`]);
        }
      });
    }
    ...
    if (data.patientName) {
      qb.whereHas("client", (query) => {
        if (data.patientName) {
          query.whereRaw("name ilike ?", [`%${data.patientName!.replaceAll(" ", "%")}%`]);
        }
      });
    }
    ...
    for (const [key, value] of statusMap.entries()) {
      ...
      const sortedValue = value.sort((a, b) => {
        ...
        if (data.orderBy === "contact") {
          qb.orderByRaw("crm_statuses.description, contact.name, opportunities.contact_date");
          return (
            a.status.description.localeCompare(b.status.description) ||
            a.contact.name.localeCompare(b.contact.name) ||
            a.contactDate.toMillis() - b.contactDate.toMillis()
      ...
      qb.with("schedule_opportunities", (cte) => {
        ...
        .with("log_opportunities", (cte) => {
          cte
            ...
            .whereRaw("opportunity_logs.economic_group_id = ?", [authCtx.group.id]);
        })
        .whereRaw(
          `(
    (opportunities.opening_date::date BETWEEN ? AND ?)
    ...
    for (const op of result) {
      ...
      statusMap.get(key)?.push({
        ...
        schedule: this.sharedService.captureGroup(op.schedule, (v) => ({
          id: v.id,
          startHour: v.startHour.plus({ hours: SharedService.GetPlatformOffset() }),
        })),
  ...
  public async storeFromSchedule(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (schedule.serviceType.type !== "A") {
        throw new BadRequestException("Tipo de Agendamento precisa ser A", 400, "E_ERR");
      }
      ...
      const status = await CrmStatus.query()
        ...
        .first();
      if (!status) {
        throw new BadRequestException("Não tem status de oportunidade válido", 400, "E_ERR");
      }
      ...
      const model = await Opportunity.create(
        {
          ...
          contact_id: schedule.holder_id ? schedule.holder_id : schedule.patient_id,
          ...
          race_id: schedule.holder_id ? undefined : schedule.patient?.patientAnimal?.race_id,
          gender: schedule.holder_id ? undefined : schedule.patient.gender,
          weight: schedule.holder_id ? undefined : schedule.patient.weight,
          castrated: schedule.holder_id ? undefined : schedule.patient?.patientAnimal?.castrated,
        },
        {
          client: trx,
        },
      ...
      await schedule.merge({ opportunity_id: model.id }).useTransaction(trx).save();
    });
  }

  public async updateClientId(
    authCtx: AuthContext,
    id: number,
    data: {
      clientId: string;
  ...
  public async executeActivity(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (activity.status !== "Aberta") {
        throw new BadRequestException("Atividade já executada ou cancelada", 400, "E_ERR");
      }
  ...
  public async reopenActivity(authCtx: AuthContext, id: number) {
    await Database.transaction(async (trx) => {
      ...
      if (!activity.executedDate) {
        throw new BadRequestException("Atividade não executada", 400, "E_INVALID");
      }
  ...
  public async searchSyncableOpportunities(
    ...
    const qb = Database.from("opportunities")
      ...
      .joinRaw(`left join patients client on client.id = opportunities.client_id`)
      .joinRaw(`left join patients contact on contact.id = opportunities.contact_id`)
  ...
  public async syncSchedules(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (!model) {
        throw this.sharedService.ResourceNotFound("Oportunidade não encontrada");
      }

      if (model.schedule_id && !["FAL", "CANC"].includes(model.schedule.serviceStatus.type)) {
        throw new BadRequestException("Oportunidade já possui agendamento", 400, "E_ERR");
      }
      ...
      if (schedule.opportunity_id) {
        throw new BadRequestException("Agendamento já contem oportunidade", 400, "E_ERR");
      }

      if (!schedule.patient_id) {
        throw new BadRequestException("Agendamento não possui paciente", 400, "E_ERR");
      }
  ...
  private async createActivityLog(model: OpportunityActivity, client?: QueryClientContract) {
