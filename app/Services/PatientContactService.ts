...
export default class PatientContactService {
  ...
  async batchStore(
    ...
    await Database.transaction(async (trx) => {
      if (data.items.some((i) => i.type === "email" && i.notGiven === false && !i.contact)) {
        throw new BadRequestException("O contato de email é obrigatório", 400, "E_MISSING");
      }
      ...
      const updateTasks = result.flat().map((elem) => {
        if (elem.type === "celular") {
          return PatientTutor.query().where("patient_id", elem.patient_id).update({
  ...
  async update(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (data.type === "celular") {
        await PatientTutor.query().where("patient_id", contact.patient_id).update({
          cellphone: data.contact,
        });
      }
  ...
  async batchUpdate(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (contacts.length < data.items.length) {
        throw new BadRequestException("Algum contato não foi encontrado", 400, "E_MISSING");
      }
