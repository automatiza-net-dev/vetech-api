...
export default class PatientExamService {
  ...
  public async store(authCtx: AuthContext, data: IPatientExamData) {
    return Database.transaction(async (trx) => {
      ...
      if (exam.product_id) {
        const activeTutor = patient.tutors.find((tutor) => tutor.$extras.pivot_is_main);
  ...
  public async destroy(unitId: string, id: string) {
    const ent = await PatientExam.query().where("business_id", unitId).where("id", id).first();
  ...
  public async deleteAttachment(unitId: string, id: string, attachmentId: string) {
    const ent = await PatientExam.query().where("business_id", unitId).where("id", id).first();
    ...
    const attachment = await ent.related("attachments").query().where("id", attachmentId).first();
