...
export default class PatientService {
	...
	public async animalsIndex(authCtx: AuthContext, data: ISearchAnimals) {
       ...
       patients.weight`),
		...
		if (data.race) {
			qb.whereRaw("races.description ilike ?", [`%${data.race}%`]);
		}
       ...
       case
		...
		for (const sale of sales) {
			...
			result.push({
				...
				missing_value:
					sale.totalValue && sale.paidValue
						? sale.totalValue.minus(sale.paidValue)
						: new Decimal(0),
				status:
  ...
  and (
	...
	async tutorCredits(_authCtx: AuthContext, tutorID: string) {
		if (!validate(tutorID)) {
			return [];
		}

		return ClientCredit.query()
			.where("client_id", tutorID)
			.where("reversed", false)
			.whereRaw("(original_value - used_value) > 0");
	}
