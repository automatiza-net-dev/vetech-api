...
export default class PatientVaccineService {
  ...
  public async store(authCtx: AuthContext, data: IPatientVaccineData) {
    return Database.transaction(async (trx) => {
      ...
      const dbCalendars = await entity.related("calendars").createMany(calendars, trx);

      const vaccineTimeline = await TimelineType.firstOrCreate(
        {
          description: protocol.vaccine.type === VaccineType.VACCINE ? "Vacinas" : "Vermifugos",
      ...
      await AnimalTimeline.create({
        ...
        timeline_info: {
          ...
          origin: protocol.vaccine.type === VaccineType.VACCINE ? "new_vaccine" : "new_vermifuge",
          patient_vaccine: {
            id: entity.id,
          },
          technician: {
  ...
  public async show(unitId: string, id: string) {
    ...
    return {
      ...
      vaccine: {
        ...
        name: [entity.importField, entity.vaccine.name].filter(Boolean).join(" - "),
      },
    };
  }
