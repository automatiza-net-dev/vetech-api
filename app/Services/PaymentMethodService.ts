...
export default class PaymentMethodService {
  ...
  async searchPartialPaymentMethods(unitId: string, data: ISearchPaymentMethods) {
  ...
  async searchCompletePaymentMethods(unitId: string, data: ISearchCompletePaymentMethods) {
    const group = await this.sharedService.getUserGroup(unitId);
  ...
  async searchTefFlags(unitId: string, data: ISearchTefFlags) {
    if (!data.type) {
      throw new BadRequestException("Informe o tipo de TEF", 400, "E_MISSING_PARAMETER");
    }
    ...
    qb.whereRaw("(economic_group_id = ? or economic_group_id is null)", [group.id]);
  ...
  async createPaymentMethod(authCtx: AuthContext, data: Omit<ICreatePaymentMethodData, "active">) {
  ...
  async createPaymentMethodFlag(unitId: string, data: ICreatePaymentMethodFlagData) {
    const group = await this.sharedService.getUserGroup(unitId);
  ...
  async updatePaymentMethodFlag(unitId: string, id: string, data: IUpdatePaymentMethodFlagData) {
    const group = await this.sharedService.getUserGroup(unitId);
    ...
    return Database.transaction(async (trx) => {
      ...
      if (updatedFlag.maxInstallments > flag.installments.length) {
        await flag.related("installments").createMany(
          Array.from(
            {
              length: (updatedFlag.maxInstallments ?? 0) - updatedFlag.installments.length,
            },
            (_, k) => ({
  ...
  async createPaymentMethodFee(unitId: string, data: ICreatePaymentMethodFeeData) {
    const group = await this.sharedService.getUserGroup(unitId);
  ...
  async updatePaymentMethodFlagInstallment(unitId: string, id: number, data: { fee: number }) {
    const group = await this.sharedService.getUserGroup(unitId);
