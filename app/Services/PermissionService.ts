...
export default class PermissionService {
  public async index(authCtx: AuthContext, data: ISearch): Promise<Array<Permission>> {
  ...
  public async store(authCtx: AuthContext, data: IPermissionData): Promise<Permission> {
  ...
  public async fetchMenu(authCtx: AuthContext) {
    const rows = await Database.from("users")
      ...
      .orderByRaw('menus.id, menus."order", screens.id, screens."name", permissions.control_id')
      .exec();
  ...
  public async show(authCtx: AuthContext, id: number): Promise<Permission> {
    ...
    if (!permission) {
      throw new ResourceNotFoundException("Permissão não encontrada", 404, "E_NOT_FOUND");
    }
