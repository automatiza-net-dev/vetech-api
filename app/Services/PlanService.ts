...
export default class PlanService {
  public async index(authCtx: AuthContext, data: ISearch): Promise<Array<Plan>> {
  ...
  public async show(authCtx: AuthContext, id: string): Promise<Plan> {
    const plan = await Plan.query().where("id", id).where("system_id", authCtx.system.id).first();

    if (!plan) {
      throw new ResourceNotFoundException("Plano não encontrado", 404, "E_NOT_FOUND");
    }

    return plan;
  }

  public async update(authCtx: AuthContext, id: string, data: IPlanData): Promise<Plan> {
