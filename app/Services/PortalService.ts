...
export default class PortalService {
  ...
  public async billingRanking(
    ...
    if (data.fromDate && data.toDate) {
      sumQb.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
      qb.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
    ...
    return {
      ...
      data: result.map((row) =>
        Object.assign(row, {
          ...
          participacao:
            total_bills > 0
              ? this.shared.formatPercentage((row.total_bills / total_bills) * 100)
              : 0,
        }),
      ),
    };
                ...
                period: {
                  dawn: {
                    ...
                    percentage: this.shared.formatPercentage(
                      new Decimal(result[0].madrugada_total ?? 0).div(sum).times(100).toNumber(),
                    ),
                  },
                  morning: {
                    total: this.shared.formatter.format(
                    ...
                    percentage: this.shared.formatPercentage(
                      new Decimal(result[0].manha_total ?? 0).div(sum).times(100).toNumber(),
                    ),
                  },
                  afternoon: {
                    total: this.shared.formatter.format(
                    ...
                    percentage: this.shared.formatPercentage(
                      new Decimal(result[0].tarde_total ?? 0).div(sum).times(100).toNumber(),
                    ),
                  },
                  night: {
                    total: this.shared.formatter.format(
                    ...
                    percentage: this.shared.formatPercentage(
                      new Decimal(result[0].noite_total ?? 0).div(sum).times(100).toNumber(),
                    ),
                  },
                },
              },
  ...
  public async attendanceRanking(
    ...
    const sumQb = Database.from("attendances")
      ...
      .joinRaw("join business_units on business_units.id = attendances.business_unit_id")
      .joinRaw(
        "join user_unit_roles on business_units.id = user_unit_roles.unit_id and user_unit_roles.user_id = ?",
        [authCtx.user.id],
      )
    ...
    const qb = Database.from("attendances")
      ...
      .whereNull("attendances.deleted_at")
      .where("business_units.environment", "P")
      .groupByRaw("economic_groups.company_name, business_units.identification, business_units.id")
    ...
    if (data.fromDate && data.toDate) {
      sumQb.whereRaw("attendances.start_date::date between ? and ?", [data.fromDate, data.toDate]);
      qb.whereRaw("attendances.start_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
    ...
    return {
      ...
      data: result.map((row) => ({
        ...
        participacaoTotal: this.shared.formatPercentage(
          new Decimal(row.qtd_total).div(new Decimal(total)).times(100).toNumber(),
        ),
        novos: row.qtd_novos,
        participacaoNovos: this.shared.formatPercentage(
          new Decimal(row.qtd_novos).div(new Decimal(novos)).times(100).toNumber(),
        ),
        recorrentes: row.qtd_recorrentes,
        participacaoRecorrentes: this.shared.formatPercentage(
          new Decimal(row.qtd_recorrentes).div(new Decimal(recorrentes)).times(100).toNumber(),
        ),
      })),
    };
  }
  ...
  public async subgroupRanking(
    ...
    const sumQb = Database.from("bills")
      ...
      .joinRaw("inner join products on products.id = product_variations.product_id")
...
sum(bill_items.total_value) as total, count(distinct bills.client_id) as clients`),
    ...
    if (data.fromDate && data.toDate) {
      sumQb.whereRaw("bills.bill_date::date between ? and ?", [data.fromDate, data.toDate]);
      qb.whereRaw("bills.bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
    ...
    const total = totalQty.length === 0 ? new Decimal(0) : new Decimal(totalQty[0].total ?? 0);
  ...
  public async invoicingProductTypeSubgroup(
    ...
    const sumQb = Database.from("bill_items")
      ...
      .joinRaw("left join products on products.id = product_variations.product_id")
      .joinRaw(
      ...
      .joinRaw("left join business_units on business_units.id = bills.business_unit_id")
      .joinRaw("left join economic_groups on economic_groups.id = business_units.economic_group_id")
  ...
  public async medianTicketByOrigin(
    ...
    join client_origins on patient_tutors.client_origin_id = client_origins.id)
      ...
      .joinRaw("inner join business_units on business_units.id = bills.business_unit_id")
      .joinRaw("join economic_groups on economic_groups.id = business_units.economic_group_id")
      .joinRaw("join user_unit_roles uur on uur.unit_id = business_units.id and uur.user_id = ?", [
        authCtx.user.id,
      ])
      .where("economic_groups.system_id", authCtx.systemID)
      .whereNull("bills.deleted_at")
    ...
    if (data.fromDate && data.toDate) {
      qb1.andWhereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }
  ...
  public async invoicingByPaymentMethod(
    ...
    if (data.fromDate && data.toDate) {
      qb1.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
      qb2.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
      qb3.whereRaw("bill_date::date between ? and ?", [data.fromDate, data.toDate]);
    }

    const result1: { total_bills: string }[] = await qb1;
    const result2: { description: string; total_payments: string }[] = await qb2;
    const result3: { description: string; total_payments: string }[] = await qb3;
    ...
    const fullResult = [...result2, ...result3].sort(
      (a, b) => Number.parseFloat(b.total_payments) - Number.parseFloat(a.total_payments),
    );
    ...
    return {
      ...
      legend: fullResult.map((elem, idx) => [
        ...
        {
          title: "Total R$",
          value: this.shared.formatter.format(new Decimal(elem.total_payments).toNumber()),
          itemStyle: { color: "" },
        },
        {
          title: "Qtd Cli",
  ...
  public async billingOverPeriod(
    ...
    const qb = Database.from("bills")
      ...
      .joinRaw("join business_units on business_units.id = bills.business_unit_id")
      .joinRaw("join user_unit_roles uur on business_units.id = uur.unit_id and uur.user_id = ?", [
        authCtx.user.id,
      ])
      .joinRaw("join economic_groups eg on business_units.economic_group_id = eg.id")
      .whereRaw("business_units.environment = 'P'", [])
    ...
    return {
      ...
      legend: result.map((r) => [
        ...
        {
          title: "Total R$",
          value: this.shared.formatter.format(new Decimal(r.total_bills).toNumber()),
          itemStyle: {
            color: "",
          },
        },
        ...
        {
          title: "Tkt Medio R$",
          value: this.shared.formatter.format(new Decimal(r.tkt_medio).toNumber()),
          itemStyle: {
            color: "",
          },
        },
