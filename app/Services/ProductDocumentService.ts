...
import ProductDocument, { type TProductDocumentType } from "App/Models/ProductDocument";
...
export default class ProductDocumentService {
  ...
  public async index(authCtx: AuthContext, data: { product?: string }) {
    ...
    const qb = ProductDocument.query()
      ...
      .whereRaw("(business_unit_id = ? or business_unit_id is null)", [authCtx.unit.id]);
  ...
  public async printDocument(
    ...
    return Database.transaction(async (trx) => {
      if (!data.billDocumentId && !data.timelineId) {
        throw new BadRequestException("Um dos dois valores precisa ser informado", 400, "E_ERR");
      }
      ...
      const bills = await Bill.query().useTransaction(trx).whereIn("id", billIds);
