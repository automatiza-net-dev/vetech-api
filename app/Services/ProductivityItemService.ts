...
export default class ProductivityItemService {
  ...
  public async searchItems(
    ...
    if ((!data.origin || data.origin === "Unidade") && !!groupID) {
      qb.whereRaw("(economic_group_id is null or economic_group_id = ?)", [groupID]);
    }
  ...
  public async searchItemProducts(
    ...
    const qb = Database.from("productivity_items")
      ...
      .where("productivity_item_products.active", data.active ? data.active === "1" : true)
  ...
  public async batchCreateItemProduct(
    ...
    await Database.transaction(async (trx) => {
      const maxQueryResult: { max: number; product_id: string }[] = await Database.from(
        "productivity_item_products",
      )
        .useTransaction(trx)
        .select(Database.raw(`coalesce(max("order"), 0) + 1 as max, product_id`))
        ...
        return elem.related("products").createMany(
          data.items
            ...
            .map((inner) => ({
              ...
              order: maxQueryResult.reduce((acc, curr) => {
                if (curr.product_id === inner.productId) {
                  return curr.max;
                }
  ...
  public async deleteProductivityItem(authCtx: AuthContext, itemID: string) {
    await Database.transaction(async (trx) => {
      const checkResult: { src: string }[] = await Database.from("productivity_item_products")
        .useTransaction(trx)
  ...
  public async deleteProductivityItemProduct(authCtx: AuthContext, itemProductID: string) {
    await Database.transaction(async (trx) => {
