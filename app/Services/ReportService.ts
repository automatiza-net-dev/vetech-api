       ...
       bills.id`),
		...
		return result
			.map((elem) => ({
				...
				productValue: elem.productValue?.toNumber() ?? 0,
				serviceValue: elem.serviceValue?.toNumber() ?? 0,
				discountValue: elem.discountValue?.toNumber() ?? 0,
				totalValue: elem.totalValue?.toNumber() ?? 0,
				paidValue: elem.paidValue?.toNumber() ?? 0,
				missingPaymentValue: (elem.totalValue ?? new Decimal(0))
					.minus(elem.paidValue ?? new Decimal(0))
					.toNumber(),
				status: elem.status,
			...
			return {
				...
				products: result
					...
					.map((p) => ({
						...
						qtyStock: Number.parseFloat(p.qtdestoque),
						minimumStock: Number.parseFloat(p.minimum_stock),
						maximumStock: Number.parseFloat(p.maximum_stock),
						suggestion: Number.parseFloat(p.sugestaocompra),
					})),
			};
		});
	}
