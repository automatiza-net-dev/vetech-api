...
export default class ScheduleServiceGroupService {
  ...
  and (schedule_service_types.economic_group_id = ?
    or schedule_service_types.economic_group_id is null)`,
      ...
      .union((builder) => {
       ...
       schedule_service_types.resume`),
          ...
          .whereRaw("business_unit_configs.business_unit_id = ?", [authCtx.unit.id])
          .whereRaw(
    ...
    return Object.keys(groups).reduce((accumulator, groupKey) => {
      ...
      if (!accumulator.find((elem) => elem.id === group_id)) {
        const rowInfo = result.find((row) => row.schedule_service_group_id === group_id);

        accumulator.push({
          id: group_id,
          description: rowInfo?.schedule_service_group_description ?? "Não informado",
  ...
  public async show(authCtx: AuthContext, id: string): Promise<ScheduleServiceGroup> {
    ...
    const exception = new ResourceNotFoundException("Recurso não encontrado", 404, "E_NOT_FOUND");
