...
export default class ScheduleServiceTypeService {
  ...
  public async index(authCtx: AuthContext, data: ISearch): Promise<Array<ScheduleServiceType>> {
  ...
  public async show(authCtx: AuthContext, id: string): Promise<ScheduleServiceType> {
    const qb = ScheduleServiceType.query().where("id", id).where("system_id", authCtx.system.id);
  ...
  public async store(
    ...
    const serviceGroup = await ScheduleServiceGroup.findOrFail(data.scheduleServiceGroupId);
  ...
  public async update(
    ...
    if (schedule.economic_group_id && schedule.economic_group_id !== authCtx.group.id) {
