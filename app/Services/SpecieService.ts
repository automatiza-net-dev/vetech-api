...
export default class SpecieService {
  ...
  async show(authCtx: AuthContext, id: string): Promise<Specie> {
    ...
    if (!specie) {
      throw new ResourceNotFoundException("Espécie não foi encontrada", 404, "E_NOT_FOUND");
    }
  ...
  async update(authCtx: AuthContext, id: string, payload: ISpecieData): Promise<Specie> {
    const specie = await this.show(authCtx, id);

    if (specie.economic_group_id && specie.economic_group_id !== authCtx.group.id) {
