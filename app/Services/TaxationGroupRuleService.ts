...
import TaxationGroupRule, { CompanyType, MovementType } from "App/Models/TaxationGroupRule";
...
export default class TaxationGroupRuleService {
  ...
  public async index(unitId: string, data: ISearch) {
    ...
    const query = TaxationGroupRule.query()
      ...
      .whereIn("taxation_group_id", (query) => {
        query.select("id").from("taxation_groups").where("economic_group_id", group.id);
      });
  ...
  public async update(unitId: string, user: User, id: string, data: ITaxationGroupRuleData) {
