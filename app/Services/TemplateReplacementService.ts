...
import createReport, { ObjectCommandResultError } from "docx-templates";
import type { SandBox, Context } from "docx-templates/lib/types";
...
type RenderTextData = Record<TemplateReplacementOrigin, ModelObject | string | null>;
...
export default class TemplateReplacementService {
  ...
  async store(authCtx: AuthContext, data: ITemplateReplacementData) {
    if (data.origin === TemplateReplacementOrigin.SYSTEM) {
      throw new BadRequestException("Você não pode criar esse tipo", 400, "E_ERR");
    }
  ...
  async update(authCtx: AuthContext, id: string, data: ITemplateReplacementData) {
  ...
  async renderText(authCtx: AuthContext, data: ITemplateReplacementParser) {
        ...
        return {
          result: this.parseTextTemplate(
            templates
              .filter((t) => t.complex)
              .reduce(
                (currText, tmpl) => currText.replaceAll(tmpl.replacer, `<li>${tmpl.replacer}</li>`),
                data.base,
              ),
            textData,
            templates,
    ...
    if (template.type === "pdf") {
      ...
      if (!fileBuffer) {
        throw new BadRequestException("Não foi possível achar o arquivo", 400, "");
      }
      ...
      const resolverSuccess = await new Promise<boolean>((res) => {
        exec(
          [
            ...
            `'${JSON.stringify(templates.filter((f) => f.complex).map((f) => f.replacer))}'`,
            `'${textData.ASSINATURA ?? ""}'`,
          ].join(" "),
      ...
      const buffer = await createReport({
        ...
        failFast: false,
        errorHandler: (idk) => {
          if (idk instanceof ObjectCommandResultError) {
            // @ts-expect-error idk
            return idk.result?.jsSandbox[idk.command] ?? idk.command;
          }
        ...
        runJs: (o: { sandbox: SandBox; ctx: Context }) => {
          ...
          if (!o.sandbox[o.sandbox.__code__ ?? "$"]) {
            newSandbox[o.sandbox.__code__ ?? "$"] = `[${o.sandbox.__code__ ?? "$"}]`;
          }

          return { modifiedSandbox: newSandbox, result: o.ctx };
        },
      });
  ...
  async reverseTextTemplateData(data: RenderTextData, templates: TemplateReplacement[]) {
    ...
    return templates.reduce(
      (map, templ) => {
        ...
        const parsedKey = templ.replacer.substring(1, templ.replacer.length - 1);
  ...
  parseTextTemplate(raw: string, data: RenderTextData, templates: TemplateReplacement[]): string {
  ...
  parseHtmlTemplate(raw: string, template: TemplateReplacement, values: string[]) {
    ...
    for (const listItem of children) {
      if (listItem.rawTagName === "li" || listItem.childNodes.some((n) => n.rawTagName === "li")) {
        if (listItem.rawTagName === "li" && listItem.textContent === template.replacer) {
          for (const value of values) {
            result.push(parse(listItem.toString().replace(template.replacer, value)));
          }
        } else {
          ...
          for (const child of listItem.childNodes) {
            if (child.rawTagName === "li" && child.textContent === template.replacer) {
              for (const value of values) {
  ...
  async fetchTutor(id: string) {
    ...
    return {
      ...
      address: [tutor.tutor?.street, tutor.tutor?.number].filter(Boolean).join(", "),
    ...
    const rows = await Database.from("bill_payments")
      ...
      .union((qb) => {
        qb.from("budget_payments")
          ...
          .joinRaw("join payment_methods on budget_payments.payment_method_id = payment_methods.id")
          .whereRaw(
      ...
      .union((qb) => {
        qb.from("budgets")
          ...
          .whereRaw("budgets.id = ? and budgets.paid_value < budgets.total_value", [
            bill.budget_id ?? v4(),
          ]);
      })
      .orderByRaw("1, 2");
