...
export default class ThirdPartyService {
  ...
  public async updateToken(
    ...
    if (!tpUser.system) {
      throw new BadRequestException("Sistema não encontrado", 400, "E_NOT_FOUND");
    }
    ...
    if (unit.economicGroup.system_id !== tpUser.system.id) {
      throw new BadRequestException("Unidade não encontrada", 400, "E_NOT_FOUND");
    }
  ...
  public async userInfo(id: string) {
    ...
    if (!model) {
      throw new BadRequestException("Usuário não encontrado", 400, "E_NOT_FOUND");
    }
  ...
  public async searchProfileAccesses(systemID: number, type?: TUserType) {
    const qb = ProfileAccess.query().where("system_id", systemID).where("active", true);
  ...
  public async syncProfileAccesses(data: { roleId: number; profileAccessIdList: number[] }) {
    await Database.transaction(async (trx) => {
      await RoleProfileAccess.query().useTransaction(trx).where("role_id", data.roleId).delete();
