...
export default class TimelineService {
  public async softDeleteRecord(authCtx: AuthContext, id: string) {
    ...
    if (!res) {
      throw new BadRequestException("Nenhum registro encontrado", 400, "E_NOT_FOUND");
    }
  ...
  public async storeEvaluation(authCtx: AuthContext, data: IPatientEvaluation) {
    return Database.transaction(async (trx) => {
      ...
      const scheduleServiceType = await ScheduleServiceType.findOrFail(data.scheduleServiceId, {
        client: trx,
      });
      ...
      const _photos = data.photos ? await Promise.all(data.photos.map(this.uploadPhoto)) : [];
  ...
  public async updatePathology(id: string, data: Omit<IAnimalPathology, "tag">) {
    const record = await AnimalTimeline.findById(id);
  ...
  public async updatePhoto(id: string, data: { title?: string; observation?: string }) {
  ...
  public async addPhotoAttachment(id: string, data: { files: MultipartFileContract[] }) {
    const record = (await AnimalTimeline.findById(id)) as {
      timeline_type: Record<string, unknown>;
      timeline_info: {
        tag: string;
    ...
    const updatedMedias = [...(record?.timeline_info?.photos ?? []), ...medias].filter(Boolean);
  ...
  public async deletePhotoAttachment(id: string, index: number) {
    ...
    const updatedMedias = (record?.timeline_info?.photos ?? []).filter((_, i) => i !== +index);
  ...
  public async deleteVaccine(authCtx: AuthContext, id: string) {
    ...
    if (resource.timeline_info?.patient_vaccine?.id) {
      await this.innerValidateDeleteVaccine(resource.timeline_info?.patient_vaccine?.id);
    }
    ...
    if (resource.timeline_info?.patient_vaccine?.id) {
      await this.innerDeleteVaccine(authCtx, resource.timeline_info?.patient_vaccine?.id);
    }
  }
  ...
  private async innerDeleteVaccine(authCtx: AuthContext, id: string) {
    ...
    await Database.from("vaccine_calendars").where("patient_vaccine_id", id).update({
      deleted_at: DateTime.now(),
      exclusion_user_id: authCtx.user.id,
    });
  }
  ...
  public async storeExam(data: ICreateAnimalExam) {
    ...
    const technician = await User.findOrFail(data.technicianId);
    const exam = await PatientExam.query().where("id", data.examId).preload("exam").firstOrFail();
  ...
  public async storeAppointment(authCtx: AuthContext, data: ICreateAppointment) {
    const timelineInfo = await TimelineType.firstOrCreate(
      {
        description: SharedService.GetAttendanceLabel(authCtx),
      },
    ...
    const serviceType = await ScheduleServiceType.findOrFail(data.scheduleServiceId);
  ...
  public async storeObservations(data: ICreateObservation) {
    ...
    const medias = data.medias ? await Promise.all(data.medias.map(this.uploadPhoto)) : [];
  ...
  public async deleteObservationMedia(id: string, index: string) {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore does have photos
    const cleanMedias = record.timeline_info?.medias?.filter((_, idx) => idx !== numericIndex);
  ...
  public async storeDeath(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (patient.patientAnimal.death) {
        throw new BadRequestException("Animal já está marcado como em óbito", 400, "E_ERR");
      }
