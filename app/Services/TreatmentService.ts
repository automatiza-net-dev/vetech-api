...
import TreatmentExecution, { TreatmentExecutionStatus } from "App/Models/TreatmentExecution";
...
export default class TreatmentService {
  ...
  public async createProductivityItem(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (!existingItem) {
        throw new BadRequestException("Item não encontrado", 400, "E_NOT_FOUND");
      }
  ...
  public async createExecution(
    ...
    return Database.transaction(async (trx) => {
      ...
      await TreatmentItem.query()
        ...
        .where("id", data.treatmentItemId)
        .update({
          scheduledQuantity: treatmentItem.scheduledQuantity + data.scheduledQuantity,
        })
        .useTransaction(trx);
      return execution;
    });
  ...
  public async executeExecution(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (execution.status !== "Ativo") {
        throw new BadRequestException("Execução já foi finalizada", 400, "E_ERR");
      }
      ...
      await TreatmentItem.query()
        ...
        .update({
          ...
          scheduledQuantity:
            treatmentItem.scheduledQuantity - (execution.scheduledQuantity - data.quantity),
        })
        .useTransaction(trx);
        ...
        timeline_info: {
          ...
          treatment: {
            ...
            execution: {
              ...
              productivityItemDescription: execution.productivityItem?.description ?? null,
  ...
  > {
    ...
    return Database.from("treatment_executions")
      ...
      .joinRaw("left join users on treatment_executions.execution_user_id = users.id")
      .groupByRaw(
  ...
  public async batchExecuteExecution(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (executions.some((elem) => elem.status !== "Ativo")) {
        throw new BadRequestException("Alguma execução já foi finalizada", 400, "E_ERR");
      }
      ...
      const updateTasks = updatedExecutions.map((elem) => {
        const item = treatmentItems.find((entry) => entry.id === elem.treatment_item_id);

        return TreatmentItem.query()
          .where("treatment_id", elem.treatment_id)
          .where("id", elem.treatment_id)
          .update({
            quantityExecuted: (item?.quantityExecuted ?? 0) + elem.quantityExecuted,
            scheduledQuantity:
              (item?.scheduledQuantity ?? 0) - (elem.scheduledQuantity - elem.quantityExecuted),
          })
          .useTransaction(trx);
      });
  ...
  public async cancelTreatment(
    ...
    return Database.transaction(async (trx) => {
      ...
      if (execution.status === "Cancelado") {
        throw new BadRequestException("Status inválido de execução", 400, "E_ERR");
      }
    ...
    const rows = await Database.from("schedule_service_types")
      ...
      .union((builder) => {
       ...
       null`),
          ...
          .whereRaw("(treatments.economic_group_id = ? or treatments.economic_group_id is null)", [
            authCtx.group.id,
          ])
          ...
          .whereIn("treatment_executions.status", ["Ativo"] as TreatmentExecutionStatus[])
  ...
  public async searchCompleteTreatments(
    ...
    return treatments.map((elem) => ({
      ...
      items: elem.items.map((inner) => ({
        ...
        productVariation: this.shared.captureGroup(inner.productVariation, (v) => ({
          id: v.id,
        ...
        departmentItems: departmentItems
          .filter((ro) => ro.treatment_id === elem.id && ro.treatment_item_id === inner.id)
          .map((r) => ({
  ...
  public async searchSomething(
    ...
    const treatments = await Treatment.query()
      .where("economic_group_id", authCtx.group.id)
      .where("business_unit_id", authCtx.unit.id)
      .whereIn("client_id", [data.client, data.holder].filter(Boolean) as string[])
    ...
    return result.map((elem) => ({
      ...
      client: elem.client
        ? {
            id: elem.client.id,
            name: elem.client.name,
            tutorPhone: elem.client.tutors.find((t) => t.tutor.cellphone)?.tutor.cellphone ?? null,
          }
        : null,
      items: elem.items.map((inner) => ({
        id: inner.id,
  ...
  public async cancelTreatmentExecution(
    ...
    await Database.transaction(async (trx) => {
      ...
      await TreatmentItem.query()
        ...
        .where("treatment_id", execution.treatment_id)
        .update({
          scheduledQuantity: treatmentItem.scheduledQuantity - execution.scheduledQuantity,
        })
        .useTransaction(trx);
    });
  }
  ...
  public async excludeTreatmentItem(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (row.status === "Confirmado") {
        throw new BadRequestException("Não é possível excluir item 'Confirmado'", 400, "E_ERR");
      }
  ...
  public async searchSyncheableTreatmentExecutions(
    ...
    const qb = Database.from("treatments")
      .select(
        ...
        (select coalesce(sum(payment_value), 0) / coalesce(nullif(sum(total_value), 0), 1) * 100 < ?
          ...
          [authCtx.unit.unitConfig.config.treatments?.minimum_percentage_paid_start_treatment ?? 0],
        ),
      )
      .joinRaw(`join (treatment_items
    join product_variations on treatment_items.product_variation_id = product_variations.id
    ...
    left join productivity_items on treatment_executions.productivity_item_id = productivity_items.id and
      ...
      .whereIn("treatments.client_id", [data.patientId, data.tutorId].filter(Boolean))
