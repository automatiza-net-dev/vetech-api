...
export default class UnavailableDayService {
  public async index(unitId: string, data: ISearch): Promise<Array<UnavailableDay>> {
    ...
    if (data.active === "true") {
      qb.whereRaw("(end_date is null or end_date::date >= now()::date)").where("active", true);
    }
  ...
  public async show(unitId: string, id: string): Promise<UnavailableDay> {
    ...
    if (!unavailableDay || unavailableDay.business_unit_id !== unitId) {
      throw new ResourceNotFoundException("Recurso não foi encontrada", 404, "E_NOT_FOUND");
    }
