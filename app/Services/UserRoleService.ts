...
export default class UserRoleService {
  ...
  public async assignUnitRoleToUser(unit: BusinessUnit, role: Role, user: User): Promise<void> {
  ...
  public async getUnitUsers(id: string, data: ISearchBusinessUnitUsers) {
    ...
    const uniqueUsers = Array.from(new Set(entities.map((entity) => entity.user)));
      ...
      return {
        ...
        roles: entities
          ...
          .map((f) => ({
            ...
            deposit: this.sharedService.captureGroup(f.deposit, (d) => ({
              id: d.id,
  ...
  public async deleteUserFromBusiness(unit: string, user: string) {
    const entities = await UserUnitRole.query().where("unit_id", unit).andWhere("user_id", user);
    ...
    try {
      ...
      throw new InternalErrorException("Erro ao executar operação", 500, "E_INTERNAL_ERROR");
    }
  }
          ...
          await UserUnitRole.create(
            {
              ...
              default_sale_deposit_id: entity.default_sale_deposit_id,
