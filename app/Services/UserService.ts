...
import { IForgotPassword, IResetPassword } from "Contracts/interfaces/ResetPassword";
...
export default class UserService {
  ...
  public async store(data: ICreateUser) {
    return Database.transaction(async (trx) => {
      ...
      if (!system) {
        throw new BadRequestException("Sistema não encontrado", 400, "E_SYSTEM_NOT_FOUND");
      }
      ...
      await unitPatient
        .related("tutor")
        .create({ corporateName: newBusinessUnit.companyName }, { client: trx });
      ...
      await newBusinessUnit.merge({ unit_patient_id: unitPatient.id }).useTransaction(trx).save();
      ...
      await newBusinessUnit.related("unitConfig").create({
        ...
        config: system.systemUrls[0].defaultConfig,
      });
      ...
      const taxationGroups = await newGroup.related("taxationGroups").createMany(
      ...
      const variationGroups = await newGroup.related("variationGroups").createMany(
  ...
  public async createUserController(
    ...
    await Database.transaction(async (trx) => {
      ...
      const units = await BusinessUnit.query().useTransaction(trx).whereIn("id", data.units);
  ...
  public async updateUserController(
    ...
    await Database.transaction(async (trx) => {
      ...
      if (!user) {
        throw new BadRequestException("Usuário não encontrado", 400, "E_USER_NOT_FOUND");
      }

      if (user.type !== "controller") {
        throw new BadRequestException("Usuário não é um controlador", 400, "E_INVALID_USER_TYPE");
      }
      ...
      await user.related("economicGroups").sync(uniqueEconomicGroups, true, trx);
    });
  }
  ...
  public async show(id: string): Promise<User> {
    ...
    if (!user) {
      throw new ResourceNotFoundException("The user was not found", 404, "E_NOT_FOUND");
    }
  ...
  public async resendConfirmationToken(email: string) {
    ...
    if (!token) {
      throw new BadRequestException("Sem token válido", 400, "E_NO_VALID_TOKEN");
    }
  ...
  public async createConfirmationToken(data: ICreateConfirmationToken): Promise<number> {
  ...
  public async confirmConfirmationToken(data: IConfirmConfirmationToken): Promise<void> {
    const token = await ConfirmationToken.query()
    ...
    await Mail.send((message) => {
      message
        ...
        .htmlView("emails/change_password", {
          ...
          url: `${authCtx.system.systemUrls.find((r) => r.url)?.url}/senha/change/${hash}`,
        });
    });
  }
  ...
  public async forgotPassword({ systemId, email }: IForgotPassword): Promise<void> {
    const user = await User.query()
      .where("email", email)
      .where("system_id", systemId)
    ...
    await Mail.send((message) => {
      message
        ...
        .htmlView("emails/reset_password", {
          email,
          url: `${user.system.systemUrls.find((r) => r.url)?.url}/senha/reset/${hash}`,
        });
    });
  }

  public async resetPassword({ hash, password }: IResetPassword): Promise<void> {
    await Database.transaction(async (trx) => {
      const validEmailChange = await UserPasswordChange.query()
        .useTransaction(trx)
        .preload("user")
