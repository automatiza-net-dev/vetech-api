...
export default class VaccineCalendarService {
  ...
  async update(authCtx: AuthContext, id: string, data: IVaccineCalendarData) {
    return Database.transaction(async (trx) => {
      ...
      if (!calendar) {
        throw new ResourceNotFoundException("Calendário de vacinação não encontrado");
      }
      ...
      const timeline = await TimelineType.firstOrCreate(
        {
          description:
            calendar.patientVaccine.vaccine.type === VaccineType.VACCINE ? "Vacinas" : "Vermifugos",
      ...
      await AnimalTimeline.create({
        ...
        timeline_info: {
          ...
          origin:
            calendar.patientVaccine.vaccine.type === VaccineType.VACCINE
              ? "vaccine_application"
        ...
        .merge({
          ...
          validUntil:
            calendar.patientVaccine.protocol.doses === data.dose
              ...
              : null,
          status: calendar.patientVaccine.protocol.doses === data.dose ? "Completo" : "Incompleto",
        })
        .save();
