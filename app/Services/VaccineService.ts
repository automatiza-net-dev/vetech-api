...
export default class VaccineService {
  ...
  public async status(
    ...
    const qb = Database.from("patient_vaccines")
      ...
      .joinRaw("join holder_dependents on p.id = holder_dependents.dependent_id")
      .joinRaw(
      ...
      .orderByRaw(
        `vaccine_calendars.dose, vaccine_calendars.created_at desc, p.name, vaccines.name, vaccine_protocols."name", vaccine_calendars.scheduling_date, vaccine_calendars.application_date`,
      );
      ...
      units: result.reduce((resultUnits, currRow) => {
        ...
        resultUnits.push({
          ...
          pacientes: result
            ...
            .reduce(
              (currPatients, pac) => {
                if (currPatients.find((f) => f.idPaciente === pac.id_paciente)) {
                  return currPatients;
                }
                ...
                currPatients.push({
                  ...
                  vacinasPaciente: result
                    .filter(
                      (f) =>
                        f.id_unidade === currRow.id_unidade && f.id_paciente === pac.id_paciente,
                    )
                    .reduce(
                      (currVacinas, vac) => {
                        if (
                          currVacinas.find((f) => f.idVacinaPaciente === vac.id_vacina_paciente)
                        ) {
                          return currVacinas;
                        }
                        ...
                        currVacinas.push({
                          ...
                          vacina: {
                            ...
                            intervaloDiasDosesProtocolo: vac.intervalo_dias_doses_protocolo,
                            ...
                            vaccineCalendar: result
                              ...
                              .map((cal) => ({
                                ...
                                statusAgendamentoVacina: cal.status_agendamento_vacina,
                              })),
                          },
                        });
  ...
  public async show(authCtx: AuthContext, id: string) {
    ...
    if (!vaccine) {
      throw new ResourceNotFoundException("Vacina não encontrada", 404, "E_NOT_FOUND");
    }
      ...
      await AnimalTimeline.updateMany(
        {
          ...
          "timeline_info.patient_vaccine.id": vaccineCalendar.patient_vaccine_id,
        },
        {
          $set: {
