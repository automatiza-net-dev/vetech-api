...
export default class VariationService {
  ...
  public async index(unitId: string, data: ISearch): Promise<Array<Variation>> {
    ...
    const qb = Variation.query().where("economic_group_id", group.id).preload("options");
  ...
  public async update(authCtx: AuthContext, id: string, data: IVariationData): Promise<Variation> {
