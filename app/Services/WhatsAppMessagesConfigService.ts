...
export default class WhatsAppMessagesConfigService {
  ...
  public async searchMessages(authCtx: AuthContext, configId: string, data: IMessagesSearch) {
  ...
  public async findConfigForTintim(accountID: string) {
    return await WhatsappMessagesConfig.query()
      .preload("businessUnit", (query) => {
        query.preload("economicGroup");
      })
      .where("tintim_client_id", accountID)
      .where("active", true)
      .whereNull("deleted_at")
      .first();
  }

  public async processTintimWebhook(data: {
    account: {
      code: string;
      name: string;
    };
    ...
    return Database.transaction(async (trx) => {
      ...
      const tutors = await config.businessUnit.economicGroup
        ...
        .whereHas("tutor", (query) => {
          query.whereRaw("(cellphone = ? or telephone = ?)", [data.phone, data.phone]);
        });
      ...
      if (data.event_type === "lead.create") {
        ...
        await config.businessUnit.economicGroup.related("patients").attach([patient.id], trx);
