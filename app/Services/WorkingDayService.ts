...
export default class WorkingDayService {
  public async index(unitId: string, user?: string): Promise<Array<WorkingDay>> {
    const unit = await BusinessUnit.findOrFail(unitId);

    const qb = unit.related("workingDays").query().orderBy("weekday_index", "asc");
  ...
  public async show(unitId: string, id: string): Promise<WorkingDay> {
    ...
    if (!workingDay || workingDay.business_unit_id !== unitId) {
      throw new ResourceNotFoundException("Jornada não foi encontrada", 404, "E_NOT_FOUND");
    }

    return workingDay;
  }

  public async store(unitId: string, data: IWorkingDayData): Promise<WorkingDay> {
    const unit = await BusinessUnit.findOrFail(unitId);
