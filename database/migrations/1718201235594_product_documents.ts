...
export default class extends BaseSchema {
  ...
  public async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer("system_product_id").unsigned().references("id").inTable("system_products");
      table.uuid("economic_group_id").references("id").inTable("economic_groups");
      table.uuid("business_unit_id").references("id").inTable("business_units");
      table.uuid("product_id").references("id").inTable("products");
      table.uuid("document_template_id").references("id").inTable("document_templates");
