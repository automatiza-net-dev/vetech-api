...
import { SS_ATTENDANCE_FINISHED, SS_LATE, SS_NOT_CONFIRMED } from "App/Models/ScheduleStatus";
...
test.group("Attendance resource", (group) => {
  ...
  test("should get all attendances with qs", async ({ assert, client }) => {
    ...
    const response = await client.get(`/attendances?${qs.toString()}`).bearerToken(token);
  ...
  test("should open attendance with patient with schedule", async ({ assert, client }) => {
    const props = await createData();
    const token = await generateJwtToken(client, {
      email: props.user.email,
      password: "102030",
  ...
  test("should open attendance with patient no schedule", async ({ assert, client }) => {
    const props = await createData();
    const token = await generateJwtToken(client, {
      email: props.user.email,
      password: "102030",
  ...
  test("should open attendance with internal observation", async ({ assert, client }) => {
    const props = await createData();
    const token = await generateJwtToken(client, {
      email: props.user.email,
      password: "102030",
