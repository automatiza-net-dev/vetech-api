...
import BudgetPayment, { TBudgetPaymentExclusionOrigin } from "App/Models/BudgetPayment";
...
test.group("Budget resource", (group) => {
  ...
  test("should return complete budget", async ({ assert, client }) => {
    ...
    const response = await client.get(`/budgets/${budget.id}`).bearerToken(token);
  ...
  test("should create budget without cashier related", async ({ assert, client }) => {
  ...
  test("should throw BadRequestException if budget is not active", async ({ assert, client }) => {
    const { user, budget, kit } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw BadRequestException if budget has invalid status when deleting", async ({
    ...
    const response = await client.delete(`/budgets/delete/${budget.id}`).bearerToken(token);
  ...
  test("should create budget payments with flag", async ({ assert, client }) => {
    const dataProps = await createData();
    const token = await generateJwtToken(client, {
      email: dataProps.user.email,
      password: "102030",
  ...
  test("should create budget payments without flag and maxParcelas", async ({ assert, client }) => {
    const dataProps = await createData();
    const token = await generateJwtToken(client, {
      email: dataProps.user.email,
      password: "102030",
  ...
  test("should throw error if new value is bigger than total value", async ({ assert, client }) => {
    const dataProps = await createData();
    const token = await generateJwtToken(client, {
      email: dataProps.user.email,
      password: "102030",
  ...
  test("should throw error if budget payment isnt 'Aberto'", async ({ assert, client }) => {
    const dataProps = await createData();
    const token = await generateJwtToken(client, {
      email: dataProps.user.email,
      password: "102030",
