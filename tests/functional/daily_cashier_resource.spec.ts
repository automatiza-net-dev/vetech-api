...
test.group("Daily cashier resource", (group) => {
  ...
  test("should return all daily cashiers", async ({ client, assert }) => {
    ...
    const response = await client.get(`/daily-cashiers?${urlParams.toString()}`).bearerToken(token);
  ...
  test("should return daily cashier info", async ({ client, assert }) => {
    ...
    const response = await client.get(`/daily-cashiers/info/${cashier.id}`).bearerToken(token);
  ...
  test("should create new daily cashier for te user", async ({ assert, client }) => {
    const { user, dailyMovement } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw BadRequestException if daily cashier is being reopen with a user with open cashier", async ({
    ...
    const response = await client.post(`/daily-cashiers/reopen/${cashier.id}`).bearerToken(token);
  ...
  test("should review a daily cashier", async ({ assert, client }) => {
    const { user, dailyMovement } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create new cashier expense (opt vals)", async ({ assert, client }) => {
    const { user, dailyMovement } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create new cashier receipt (opt vals)", async ({ assert, client }) => {
    const { user, dailyMovement } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
