...
test.group("Deposit resource", (group) => {
  ...
  test("should search deposits", async ({ assert, client }) => {
    ...
    const result = await client.get("/deposits/search-deposits").bearerToken(token);
  ...
  test("should show deposit", async ({ assert, client }) => {
    ...
    const result = await client.get(`/deposits/show-deposit/${deposit.id}`).bearerToken(token);
  ...
  test("should search deposit movements", async ({ assert, client }) => {
    ...
    const result = await client.get("/deposits/search-deposit-movements").bearerToken(token);
