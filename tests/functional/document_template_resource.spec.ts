...
test.group("Document template resource", (group) => {
  ...
  test("should throw ResouceNotFound if no document was found", async ({ client, assert }) => {
    ...
    const response = await client.get(`/document-templates/${v4()}`).bearerToken(token);
    const body = response.body();
  ...
  test("should return given document", async ({ client, assert }) => {
    ...
    const response = await client.get(`/document-templates/${template.id}`).bearerToken(token);
    const body = response.body();
  ...
  test("should delete a document template", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/document-templates/${template.id}`).bearerToken(token);
