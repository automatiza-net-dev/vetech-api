...
test.group("Dre Groups resource", (group) => {
  ...
  test("should search dre groups", async ({ assert, client }) => {
    ...
    const result = await client.get(`/dre-groups/search?${qs.toString()}`).bearerToken(token);
  ...
  test("should not create dre groups with duplicate sequence", async ({ assert, client }) => {
  ...
  test("should not update dre group for duplicate sequence", async ({ assert, client }) => {
    const { user, system, business } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should delete dre groups", async ({ assert, client }) => {
    ...
    const result = await client.delete(`/dre-groups/delete/${group.id}`).bearerToken(token);
  ...
  test("should create dre group planning with items ", async ({ assert, client }) => {
  ...
  test("should create dre group planning with duplicate items ", async ({ assert, client }) => {
    const { user, accountPlan } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should update dre group planning with items ", async ({ assert, client }) => {
  ...
  test("should update dre group planning with duplicate items ", async ({ assert, client }) => {
