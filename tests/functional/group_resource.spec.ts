...
test.group("Group resource", (group) => {
  ...
  test("should throw ResourceNotFound for invalid group", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should return valid group", async ({ assert, client }) => {
    ...
    const response = await client.get(`/groups/${resGroup.id}`).bearerToken(token);
  ...
  test("should soft delete the group", async ({ assert, client }) => {
    ...
    const response = await client.delete(`/groups/${resGroup.id}`).bearerToken(token);
