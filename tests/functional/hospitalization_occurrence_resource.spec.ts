...
test.group("Hospitalization occurrence resource", (group) => {
  ...
  const createData = async () => {
    ...
    const prescription = await hospitalization.related("medicalPrescriptions").create({
  ...
  test("should create hospitalization occurrence", async ({ assert, client }) => {
    const { user, hospitalization, occurrence, prescription } = await createData();
  ...
  test("should throw NotFoundException for invalid hospitalization", async ({ assert, client }) => {
    const { user, occurrence, prescription, hospOccurrence } = await createData();
  ...
  test("should update hospitalization occurrence", async ({ assert, client }) => {
    const { user, hospitalization, occurrence, prescription, hospOccurrence } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });
  ...
  test("should delete hospitalization occurrence", async ({ assert, client }) => {
