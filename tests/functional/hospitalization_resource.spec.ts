...
test.group("Hospitalization resource", (group) => {
  ...
  test("should return a list of hospitalizations", async ({ assert, client }) => {
  ...
  test("should return a list of completed hospitalizations", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const response = await client.get(`/hospitalizations/completed`).bearerToken(token);
  ...
  test("should return ResourceNotFoundException if hospitalization does not exists", async ({
    ...
    const response = await client.get(`/hospitalizations/${v4()}`).bearerToken(token);
  ...
  test("should return hospitalization", async ({ client, assert }) => {
    ...
    const response = await client.get(`/hospitalizations/${hospitalization.id}`).bearerToken(token);
