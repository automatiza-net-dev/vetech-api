...
test.group("Marketing Campaign resource", (group) => {
  ...
  test("should get all marketing campaigns", async ({ assert, client }) => {
    ...
    const result = await client.get(`/marketing-campaigns?${qs.toString()}`).bearerToken(token);
  ...
  test("should create marketing campaign without origins", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const result = await client
      ...
      .json({
        ...
        endDate: "2024-12-31",
        investmentValue: 100,
      } as Omit<Parameters<MarketingCampaignService["store"]>[1], "startDate" | "endDate">)
      .bearerToken(token);

    assert.equal(201, result.status());
  });

  test("should create marketing campaign with origins", async ({ assert, client }) => {
  ...
  test("should update marketing campaign without origins", async ({ assert, client }) => {
    ...
    const result = await client
      ...
      .json({
        ...
        investmentValue: 100,
        active: true,
      } as Omit<Parameters<MarketingCampaignService["update"]>[1], "startDate" | "endDate">)
      .bearerToken(token);
  ...
  test("should update marketing campaign adding origins", async ({ assert, client }) => {
  ...
  test("should update marketing campaign removing origins", async ({ assert, client }) => {
    const { user, campaign, origin } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should update marketing campaign swapping origins", async ({ assert, client }) => {
    const { user, campaign, origin } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
