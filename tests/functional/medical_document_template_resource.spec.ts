...
test.group("Medical document template resource", (group) => {
  ...
  test("should get all medical document templates", async ({ client, assert }) => {
    ...
    const response = await client.get("/medical-document-templates").bearerToken(token);
  ...
  test("should throw ResouceNotFound if no medical document was found", async ({
    ...
    const response = await client.get(`/medical-document-templates/${v4()}`).bearerToken(token);
    const body = response.body();
  ...
  test("should create a new medical document template", async ({ client, assert }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should update a medical document template", async ({ client, assert }) => {
    const { user, template } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should delete a medical document template", async ({ client, assert }) => {
    const { user, template } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
