...
test.group("Medical prescription resource", (group) => {
  ...
  test("should return all medical prescriptions", async ({ assert, client }) => {
    ...
    const response = await client.get(`/medical-prescriptions`).bearerToken(token);

    assert.equal(200, response.status());
    assert.isArray(response.body());
  });

  test("should create medical prescriptions (PR)", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create medical prescriptions (MR)", async ({ assert, client }) => {
  ...
  test("should create medical prescriptions (FR)", async ({ assert, client }) => {
    const { user, unit, drug } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create medical prescriptions (F_)", async ({ assert, client }) => {
    const { user, unit, drug } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create medical prescriptions (M_)", async ({ assert, client }) => {
    const { user, unit, drug } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw ResourceNotFound if no prescription was found", async ({ client, assert }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const response = await client.get(`/medical-prescriptions/${v4()}`).bearerToken(token);
