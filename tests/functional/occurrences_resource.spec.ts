...
test.group("Occurrences resource", (group) => {
  ...
  test("should throw ResouceNotFound if no occurence was found", async ({ client, assert }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const response = await client.get(`/occurrences/${v4()}`).bearerToken(token);
  ...
  test("should return given occurence", async ({ client, assert }) => {
    ...
    const response = await client.get(`/occurrences/${occurence.id}`).bearerToken(token);
    const body = response.body();
  ...
  test("should delete a occurence", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/occurrences/${occurence.id}`).bearerToken(token);
