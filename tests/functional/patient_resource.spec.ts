...
test.group("Patient resource", (group) => {
  ...
  test("should return list of patients from clinic", async ({ client, assert }) => {
  ...
  test("should return list of patients from clinic", async ({ client, assert }) => {
    ...
    qs.append("tag", "1");
    const response = await client.get(`/patients/animals?${qs.toString()}`).bearerToken(token);
  ...
  test("should throw NotFoundException if no valid patient is found", async ({
    ...
    const response = await client.get(`/patients/${patient.id}`).bearerToken(token);
  ...
  test("should soft delete a patient with a singled related business unit", async ({
    ...
    const response = await client.delete(`/patients/${patient.id}`).bearerToken(token);
  ...
  test("should get all tutors", async ({ client, assert }) => {
    ...
    const qs = new URLSearchParams();
    qs.append("patientId", v4());
    qs.append("patient", v4());

    const response = await client.get(`/patient-tutors?${qs.toString()}`).bearerToken(token);
  ...
  test("should show tutor", async ({ client, assert }) => {
    ...
    const response = await client.get(`/patient-tutors/${patient.id}`).bearerToken(token);
  ...
  test("should get non related patients", async ({ client, assert }) => {
    ...
    const response = await client.get(`/patient-tutors/nr/${holder.id}`).bearerToken(token);
  ...
  test("should create patient and tutor (fast) 2", async ({ client, assert }) => {
  ...
  test("should search for suppliers", async ({ client, assert }) => {
    ...
    const response = await client.get(`/patient-suppliers?${params.toString()}`).bearerToken(token);
  ...
  test("should show supplier", async ({ client, assert }) => {
    ...
    const response = await client.get(`/patient-suppliers/${holder.id}`).bearerToken(token);
  ...
  test("should return existing false for invalid document", async ({ client, assert }) => {
    ...
    const response = await client.get(`/patients/check-document/invalid-email`).bearerToken(token);
  ...
  test("should return existing true for valid document and false for in usage", async ({
    ...
    const response = await client.get(`/patients/check-document/74069759000167`).bearerToken(token);
  ...
  test("should return existing true for valid document and true for in usage", async ({
    ...
    const response = await client.get(`/patients/check-document/94562755000123`).bearerToken(token);
