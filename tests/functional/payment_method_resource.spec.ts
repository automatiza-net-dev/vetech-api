...
test.group("Payment method resource", (group) => {
  ...
  test("should create a payment method flag", async ({ client, assert }) => {
    const { user, paymentMethod, checkingAccount, tefAcq, tefFlag } = await createData();
  ...
  test("should list for checking account default", async ({ client, assert }) => {
  ...
  test("should update checking account default", async ({ client, assert }) => {
    ...
    const response = await client
      ...
      .json({
        items: [{ id: row.id, checkingAccountId: checkingAccount.id, active: true }],
      })
      .bearerToken(token);
