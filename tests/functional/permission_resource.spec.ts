...
test.group("Permission resource", (group) => {
  ...
  test("should return a list of all permissions", async ({ client, assert }) => {
  ...
  test("should return a permission", async ({ client, assert }) => {
    ...
    const response = await client.get(`/permissions/${permission.id}`).bearerToken(token);
  ...
  test("should delete a permission", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/permissions/${permission.id}`).bearerToken(token);
