...
test.group("Product resource", (group) => {
  ...
  const createData = async () => {
    ...
    await kit.related("items").create({
  ...
  test("should create a product", async ({ client, assert }) => {
    const { user, variationGroup, groupEntity, subgroupEntity, unit, taxationGroup } =
      await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should create a product with minimal info", async ({ client, assert }) => {
    const { user, subgroupEntity, taxationGroup, variationGroup } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });
  ...
  test("should create a product with specific business price", async ({ client, assert }) => {
    const { user, variationGroup, business, groupEntity, subgroupEntity, unit, taxationGroup } =
      await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw ResourceNotFound if no product is found", async ({ client, assert }) => {
  ...
  test("should update a product", async ({ client, assert }) => {
    const { user, product, subgroupEntity, groupEntity, unit, taxationGroup } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });
  ...
  test("should not soft delete if has dangling references", async ({ client, assert }) => {
    const { user, product } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should soft delete a product", async ({ client, assert }) => {
    const { user, product, ...props } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });

    await Database.from("kit_items").delete().where("product_variation_id", props.variation.id);
