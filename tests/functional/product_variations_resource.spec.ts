...
test.group("Product variations resource", (group) => {
  ...
  test("should return a list of product variations", async ({ client, assert }) => {
  ...
  test("should throw ResourceNotFound if no product variation is found", async ({
    ...
    const response = await client.get(`/product-variations/${v4()}`).bearerToken(token);
  ...
  test("should return given product variation", async ({ client, assert }) => {
    ...
    const response = await client.get(`/product-variations/${variation.id}`).bearerToken(token);
  ...
  test("should soft delete a product variation", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/product-variations/${variation.id}`).bearerToken(token);
