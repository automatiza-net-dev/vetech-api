...
test.group("Schedule service group resource", (group) => {
  ...
  test("should return a list of schedule service groups", async ({ assert, client }) => {
    ...
    const response = await client.get("/schedule-service-groups").bearerToken(token);
  ...
  test("should throw ResourceNotFoundException if no schedule service group was found", async ({
    ...
    const response = await client.get(`/schedule-service-groups/${v4()}`).bearerToken(token);
  ...
  test("should return schedule", async ({ assert, client }) => {
    ...
    const response = await client.get(`/schedule-service-groups/${schedule.id}`).bearerToken(token);
