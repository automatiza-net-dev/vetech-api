...
test.group("Scheduling resource", (group) => {
  ...
  const createData = async () => {
    ...
    const treatmentItem = await TreatmentItem.create({
      economic_group_id: group.id,
      business_unit_id: business.id,
    ...
    const treatmentExecution = await TreatmentExecution.create({
      economic_group_id: group.id,
      business_unit_id: business.id,
  ...
  const createWorkingDay = async (user: User, unit: BusinessUnit, start: string, end: string) => {
  ...
  test("should fetch user info", async ({ assert, client }) => {
    ...
    const result = await client.get(`/schedules/user?${qs.toString()}`).bearerToken(token);

    assert.equal(200, result.status());
  });

  test("should return home content (not confirmed)", async ({ assert, client }) => {
    const { user, serviceType, business, holder } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should return home content (confirmed)", async ({ assert, client }) => {
    ...
    const result = await client.get(`/schedules/home?${urlParams.toString()}`).bearerToken(token);
  ...
  test("should throw BadRequestException if matches unavailable time", async ({
    ...
    const result = await client
      ...
      .json({
        scheduleServiceTypeId: serviceType.id,
        scheduleStatusId: status.id,
        startHour: new Date(`2022-${d.getMonth() + 1}-${d.getDate()} 08:00:00`).toISOString(),
        endHour: new Date(`2022-${d.getMonth() + 1}-${d.getDate()} 08:50:00`).toISOString(),
        userId: user.id,
      })
      .bearerToken(token);
  ...
  test("should create scheduling for on duty user", async ({ assert, client }) => {
    const { user, status, serviceType } = await createData();
  ...
  test("should create scheduling with executions", async ({ assert, client }) => {
    const {
      user,
      status,
      serviceType,
  ...
  test("should return users with schedule", async ({ assert, client }) => {
    ...
    const result = await client.get("/schedules/with-schedule").bearerToken(token);

    assert.equal(200, result.status());
    assert.isArray(result.body());
  });

  test("should return users with schedule and on duty", async ({ assert, client }) => {
    const { user, serviceType, business, holder } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const result = await client.get(`/schedules/with-schedule?${qs.toString()}`).bearerToken(token);
  ...
  test("should return schedule status changes", async ({ assert, client }) => {
    ...
    const result = await client.get(`/schedules/status-changes/${schedule.id}`).bearerToken(token);
  ...
  test("should return schedule", async ({ assert, client }) => {
    ...
    const result = await client.get(`/schedules/${schedule.id}`).bearerToken(token);

    assert.equal(200, result.status());
  });

  test("should get patient historic", async ({ assert, client }) => {
    const { user, patient, holder, business, serviceType, reason } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });
    ...
    const result = await client.get(`/schedules/historic/${patient.id}`).bearerToken(token);
