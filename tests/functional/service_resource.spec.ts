...
test.group("Service resource", (group) => {
  ...
  test("should throw ResourceNotFound if no service is found", async ({ client, assert }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should return given service", async ({ client, assert }) => {
    ...
    const response = await client.get(`/services/${service.id}`).bearerToken(token);
  ...
  test("should update a service", async ({ client, assert }) => {
    const { user, service, subgroupEntity, unit, taxationGroup } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    });
  ...
  test("should soft delete a service", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/services/${service.id}`).bearerToken(token);
