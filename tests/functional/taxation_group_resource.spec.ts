...
test.group("Taxation group resource", (group) => {
  ...
  test("should throw NotFoundException if no group is found", async ({ client, assert }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
    ...
    const response = await client.get(`/taxation-groups/${v4()}`).bearerToken(token);
  ...
  test("should return taxation group by id", async ({ client, assert }) => {
    ...
    const response = await client.get(`/taxation-groups/${taxation.id}`).bearerToken(token);
  ...
  test("should delete taxation group", async ({ client, assert }) => {
    ...
    const response = await client.delete(`/taxation-groups/${taxation.id}`).bearerToken(token);
