...
test.group("Template replacement resource", (group) => {
  ...
  test("should return all template replacements", async ({ assert, client }) => {
    ...
    const response = await client.get(`/template-replacements`).bearerToken(token);

    assert.equal(200, response.status());
    assert.isArray(response.body());
  });

  test("should throw BadRequestException creating system origin", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should replace text with bill contract data", async ({ assert, client }) => {
    ...
    const dailyMovement = await DailyMovement.create({
      ...
      user_who_opened_id: ctx.user.id,
    ...
    const paymentMethod = await PaymentMethod.create({
      economicGroupId: ctx.business.economicGroupId,
    ...
    const templates = await TemplateReplacement.createMany([
      {
        economic_group_id: ctx.business.economicGroupId,
        origin: TemplateReplacementOrigin.CONTRACTS,
        attribute: "date",
        replacer: "[CONTRATO_PAGAMENTOS]",
      },
    ]);
