...
test.group("Third party resource", (group) => {
  ...
  const createData = async (systemName?: string) => {
    const { user, business, group, system: defaultSystem } = await userBootstrap(systemName);

    const tpUser = await ThirdPartyUser.create({
      name: "Test",
    });

    await defaultSystem.related("systemUrls").create({
    ...
    await user.merge({ system_id: defaultSystem.id }).save();
    ...
    await group.merge({ system_id: defaultSystem.id }).save();
  ...
  const createToken = async (
    ...
    const response = await client.post(`/external/authenticate-${systemName.toLowerCase()}`).json({
  ...
  test("should extended authenticate", async ({ client, assert }) => {
    ...
    const response = await client.post("/external/extended-authenticate-vetech").json({
  ...
  test("should get unit info", async ({ client, assert }) => {
    ...
    const response = await client.get(`/external/business/${business.id}`).bearerToken(token);
  ...
  test("should get user info", async ({ client, assert }) => {
    ...
    const response = await client.get(`/external/user/${user.id}`).bearerToken(token);
  ...
  test("should update user token", async ({ client, assert }) => {
    const { tpPermission, user, business } = await createData("Vetech");
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
      systemName: "Vetech",
    });
    const tpToken = await createToken(client, tpPermission, "vetech");

    const response = await client
      .post(`/external/update-token`)
      .json({
        userToken: token,
        unitId: business.id,
      })
