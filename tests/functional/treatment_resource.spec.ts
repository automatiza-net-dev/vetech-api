...
test.group("Treatment resource", (group) => {
  ...
  test("should create a treatment item (with kit)", async ({ assert, client }) => {
  ...
  test("should create a treatment productivity item", async ({ assert, client }) => {
  ...
  test("should batch create a treatment execution", async ({ assert, client }) => {
    const { user, treatment, item, schedule } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should fetch treatments (with qs)", async ({ assert, client }) => {
    ...
    const response = await client.get(`/treatments/search?${qs.toString()}`).bearerToken(token);
  ...
  test("should throw BadRequestException if searching items with no treatment", async ({
    ...
    const response = await client.get(`/treatments/search-items`).bearerToken(token);
  ...
  test("should throw BadRequestException if searching executions with no treatment", async ({
    ...
    const response = await client.get(`/treatments/search-executions`).bearerToken(token);
  ...
  test("should throw BadRequestException if searching complete treatments with no treatment", async ({
    ...
    const response = await client.get(`/treatments/search-complete`).bearerToken(token);
