...
test.group("User resource", (group) => {
  ...
  test("should throw not found exception if no user was found", async ({ client, assert }) => {
  ...
  test("should return existing false for no email in database", async ({ client, assert }) => {
  ...
  test("should throw BadRequestException for not active token", async ({ client, assert }) => {
    const token1 = await ConfirmationToken.create({
      email: "user1@mail.com",
      code: "0001",
      expiresAt: DateTime.now().plus({ days: 1 }),
  ...
  test("should throw BadRequestException for expired token", async ({ client, assert }) => {
    const token1 = await ConfirmationToken.create({
      email: "user1@mail.com",
      code: "0001",
  ...
  test("should throw BadRequestException if email is already on platform", async ({
    ...
    const { user } = await createUser();
    const response = await client.get(`/users/resend-confirmation/${user.email}`);

    assert.equal(400, response.status());
  });

  test("should throw BadRequestException if no token was found", async ({ client, assert }) => {
    const response = await client.get(`/users/resend-confirmation/user1@mail.com`);
  ...
  test("should resend confirmation email", async ({ client, assert }) => {
    ...
    const response = await client.get(`/users/resend-confirmation/${token1.email}`);
  ...
  test("should throw BadRequestException if there is a active change password link", async ({
    ...
    const response = await client.post(`/users/start-change-password`).bearerToken(token);
  ...
  test("should throw BadRequestException if hash is expired", async ({ client, assert }) => {
    const { user, group, system, business } = await createUser();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw BadRequestException if hash is completed", async ({ client, assert }) => {
    const { user, group, system, business } = await createUser();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should fetch user controllers", async ({ client, assert }) => {
    ...
    const response = await client.get(`/users/fetch-user-controllers`).bearerToken(token);
