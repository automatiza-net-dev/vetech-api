...
test.group("Vaccine resource", (group) => {
  ...
  test("should create vaccine without description", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should throw NotFoundException if no vaccine is found", async ({ assert, client }) => {
    const { user } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should show vaccine", async ({ assert, client }) => {
    ...
    const response = await client.get(`/vaccines/${vaccine.id}`).bearerToken(token);
  ...
  test("should update vaccine without description", async ({ assert, client }) => {
    const { user, vaccine } = await createData();
    const token = await generateJwtToken(client, {
      email: user.email,
      password: "102030",
  ...
  test("should soft delete vaccine", async ({ assert, client }) => {
    ...
    const response = await client.delete(`/vaccines/${vaccine.id}`).bearerToken(token);
